...
public class GestorCorreoNotificacionesEni {
	...
	public static final Logger log = Logger.getLogger(GestorCorreoNotificacionesEni.class);
	
	private Session session;

	private static GestorCorreoNotificacionesEni instance = null;
	
	protected GestorCorreoNotificacionesEni() {
		super();
	...
	public static GestorCorreoNotificacionesEni getInstance() {
		if(instance == null) {
	    	instance = new GestorCorreoNotificacionesEni();
	    }
		return instance;
	}
	...
	private Session getSession(){
		if (session == null) {
			...
			String mailSmtpUser =  mapaPropiedades.get("mail_usuario_notificaciones_eni");
			...
			//props.setProperty("mail.smtp.sasl.authorizationid", null);
	
	        String password = mapaPropiedades.get("mail_password_notificaciones_eni");
	...
	public ResultBean enviarMail(CorreoRequest correo){
		...
		try {
			...
			mimeMessage.setFrom(new InternetAddress((correo.getFrom() != null && !correo.getFrom().trim().isEmpty())? correo.getFrom() :  mapaPropiedades.get("from_notif")));
