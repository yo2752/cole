...
public class GestorCorreoBajas {
	...
	public static final Logger log = Logger.getLogger(GestorCorreoBajas.class);
	
	private Session session;

	private static GestorCorreoBajas instance = null;
	
	protected GestorCorreoBajas() {
		super();
	...
	public static GestorCorreoBajas getInstance() {
		if(instance == null) {
	    	instance = new GestorCorreoBajas();
	    }
		return instance;
	}
	...
	private Session getSession(){
		if (session == null) {
			...
			String mailSmtpUser =  mapaPropiedades.get("mail_usuario_bajas");
	        ...
	        String password = mapaPropiedades.get("mail_password_bajas");
	...
	public ResultBean enviarMail(CorreoRequest correo){
		...
		try {
			...
			mimeMessage.setFrom(new InternetAddress((correo.getFrom() != null && !correo.getFrom().trim().isEmpty()) ? correo.getFrom() :  mapaPropiedades.get("from_bajas")));
