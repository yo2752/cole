...
public class GestorCorreo {
	...
	public static final Logger log = Logger.getLogger(GestorCorreo.class);
	
	private Session session;

	private static GestorCorreo instance = null;
	
	protected GestorCorreo() {
		super();
	...
	public static GestorCorreo getInstance() {
		if(instance == null) {
	    	instance = new GestorCorreo();
	    }
		return instance;
	}
	...
	private Session getSession(){
		if (session == null) {
			...
			String mailSmtpUser =  mapaPropiedades.get("mail_usuario_oegam");
		...
		//	props.setProperty("https.protocols", "TLSv1.2");
	        String password = mapaPropiedades.get("mail_password_oegam");
	...
	public ResultBean enviarMail(CorreoRequest correo){
		...
		try {
			...
			mimeMessage.setFrom(new InternetAddress((correo.getFrom() != null && !correo.getFrom().trim().isEmpty()) ? correo.getFrom() :  mapaPropiedades.get("from_oegam")));
			...
			if(correo.getAdjuntos()!=null){
				for (FicheroAdjunto adjunto: correo.getAdjuntos()) {
					...
					DataSource source = new ByteArrayDataSource(adjunto.getFicheroByte(), contentType!=null?contentType:"text/plain");
//					char[] prueba = {((char)adjunto.getFicheroByte()[2]) , ((char)adjunto.getFicheroByte()[3]) , ((char)adjunto.getFicheroByte()[4])};
//					String.valueOf(prueba);
