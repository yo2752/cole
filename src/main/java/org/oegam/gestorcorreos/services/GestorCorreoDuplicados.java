...
public class GestorCorreoDuplicados {
	...
	public static final Logger log = Logger.getLogger(GestorCorreoDuplicados.class);
	
	private Session session;

	private static GestorCorreoDuplicados instance = null;
	
	protected GestorCorreoDuplicados() {
		super();
	...
	public static GestorCorreoDuplicados getInstance() {
		if(instance == null) {
	    	instance = new GestorCorreoDuplicados();
	    }
		return instance;
	}
	...
	private Session getSession(){
		if (session == null) {
			...
			String mailSmtpUser =  mapaPropiedades.get("mail_usuario_duplicados");
		...
		//	props.setProperty("https.protocols", "TLSv1.2");
	        ...
	        String password = mapaPropiedades.get("mail_password_duplicados");
	...
	public ResultBean enviarMail(CorreoRequest correo){
		...
		try {
			...
			mimeMessage.setFrom(new InternetAddress((correo.getFrom() != null && !correo.getFrom().trim().isEmpty())? correo.getFrom() :  mapaPropiedades.get("from_duplicados")));
