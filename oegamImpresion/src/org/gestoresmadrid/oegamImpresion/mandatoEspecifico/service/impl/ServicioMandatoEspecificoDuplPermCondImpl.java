...
@Service
public class ServicioMandatoEspecificoDuplPermCondImpl implements ServicioMandatoEspecificoDuplPermCond {

	private static final long serialVersionUID = 3698701411902288650L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioMandatoEspecificoDuplPermCondImpl.class);
	...
	public ResultadoImpresionBean imprimirMandatoEspecifico(List<BigDecimal> listaExpediente) {
		...
		try {
			...
			log.error("Ha sucedido un error a la hora de imprimir los mandatos especificos de Duplicado de Permiso de Conducir, error: ", e);
			resultado.setError(Boolean.TRUE);
			resultado.setMensaje("Ha sucedido un error a la hora de imprimir los mandatos especificos de Duplicado de Permiso de Conducir.");
		}
		return resultado;
	}
	...
	private ArrayList<CampoPdfBean> rellenarPdf(DuplicadoPermisoConducirVO duplicado, Set<String> camposPlantilla, ContratoVO contrato) {
		...
		rellenarGestor(duplicado, camposPlantilla, camposFormateados, contrato);

		rellenarAsuntos(duplicado, camposPlantilla, camposFormateados);
	...
	private void rellenarGestor(DuplicadoPermisoConducirVO duplicado, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados, ContratoVO contrato) {
	...
	private void rellenarAsuntos(DuplicadoPermisoConducirVO duplicado, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados) {
		if (camposPlantilla.contains(ConstantesPDF.ID_ASUNTO1_EXPLICACION)) {
			camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_ASUNTO1_EXPLICACION, "DUPLICADO PERMISO CONDUCIR", false, false, ConstantesPDF._11));
		}

		if (camposPlantilla.contains(ConstantesPDF.ID_ASUNTO2)) {
			if (duplicado.getTipoDuplicado() != null) {
				camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_ASUNTO2, TiposDuplicadosPermisos.convertirNombre(duplicado.getTipoDuplicado()), false, false, ConstantesPDF._11));
			}
		}
	}
	...
	private IntervinienteIntergaVO buscarInterviniente(DuplicadoPermisoConducirVO duplicado) {
		return intervinientePermInterDao.getIntervinienteTrafico(duplicado.getNumExpediente(), null, null, TipoTramiteInterga.Duplicado_Permiso_Conducir.getValorEnum());
	}
