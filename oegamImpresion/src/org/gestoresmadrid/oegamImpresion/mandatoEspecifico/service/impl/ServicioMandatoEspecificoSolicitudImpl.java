...
@Service
public class ServicioMandatoEspecificoSolicitudImpl implements ServicioMandatoEspecificoSolicitud {

	private static final long serialVersionUID = -373326651806600394L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioMandatoEspecificoSolicitudImpl.class);
	...
	public ResultadoImpresionBean imprimirMandatoEspecifico(List<BigDecimal> listaExpediente) {
		...
		try {
			...
			log.error("Ha sucedido un error a la hora de imprimir los mandatos especificos de Solicitud, error: ", e);
			resultado.setError(Boolean.TRUE);
			resultado.setMensaje("Ha sucedido un error a la hora de imprimir los mandatos especificos de Solicitud.");
		}
		return resultado;
	}
	...
	private void rellenarTitular(TramiteTrafSolInfoVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados) {
	...
	private void rellenarAsuntos(TramiteTrafSolInfoVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados) {
		if (camposPlantilla.contains(ConstantesPDF.ID_ASUNTO1_EXPLICACION)) {
			camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_ASUNTO1_EXPLICACION, "Solicitud de Informes: ", false, false, ConstantesPDF._11));
		}

		if (camposPlantilla.contains(ConstantesPDF.ID_ASUNTO2)) {
			String asuntoExplicacion = "";
			int cont = 1;
			if (tramite.getSolicitudes() != null) {
				for (TramiteTrafSolInfoVehiculoVO solicitudVehiculo : tramite.getSolicitudes()) {
					if (solicitudVehiculo.getVehiculo() != null) {
						if (StringUtils.isNotBlank(solicitudVehiculo.getVehiculo().getMatricula())) {
							asuntoExplicacion += solicitudVehiculo.getVehiculo().getMatricula();
						} else if (StringUtils.isNotBlank(solicitudVehiculo.getVehiculo().getBastidor())) {
							asuntoExplicacion += solicitudVehiculo.getVehiculo().getBastidor();
						} else if (StringUtils.isNotBlank(solicitudVehiculo.getVehiculo().getNive())) {
							asuntoExplicacion += solicitudVehiculo.getVehiculo().getNive();
						}
						if (cont < tramite.getSolicitudes().size()) {
							asuntoExplicacion += ", ";
						}
						cont++;
					}
			...
			camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_ASUNTO2, asuntoExplicacion, false, false, ConstantesPDF._11));
		}
	}
