...
@Service
public class ServicioMandatoEspecificoCtitImpl implements ServicioMandatoEspecificoCtit {

	private static final long serialVersionUID = 2471947495908414348L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioMandatoEspecificoCtitImpl.class);
	...
	@Override
	public ResultadoImpresionBean imprimirMandatoEspecifico(List<BigDecimal> listaExpediente, String tipoInterviniente) {
		...
		try {
			...
			log.error("Ha sucedido un error a la hora de imprimir los mandatos especificos de Ctit, error: ", e);
			resultado.setError(Boolean.TRUE);
			resultado.setMensaje("Ha sucedido un error a la hora de imprimir los mandatos especificos de Ctit.");
		}
		return resultado;
	}
		...
		if (interviniente != null) {
			...
			if (interviniente.getDireccion() != null) {
				...
				if (camposPlantilla.contains(ConstantesPDF.ID_PROVINCIA_ENTIDAD)) {
					String provincia = servicioDireccion.obtenerNombreProvincia(interviniente.getDireccion().getIdProvincia());
	...
	private void rellenarAsuntos(TramiteTrafTranVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados) {
		if (camposPlantilla.contains(ConstantesPDF.ID_ASUNTO1_EXPLICACION)) {
			camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_ASUNTO1_EXPLICACION, "TRANSFERENCIA: ", false, false, ConstantesPDF._11));
		}

		if (camposPlantilla.contains(ConstantesPDF.ID_ASUNTO2)) {
			if (tramite.getVehiculo() != null) {
				String asunto = "";
				if (tramite.getVehiculo().getBastidor() != null) {
					asunto = "BAST: " + tramite.getVehiculo().getBastidor().toString();
				}
				if (tramite.getVehiculo().getMatricula() != null) {
					asunto = asunto + "  MAT: " + tramite.getVehiculo().getMatricula().toString();
				}
				if (tramite.getVehiculo() != null) {
					camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_ASUNTO2, asunto, false, false, ConstantesPDF._11));
				}
			}
		}
	}
