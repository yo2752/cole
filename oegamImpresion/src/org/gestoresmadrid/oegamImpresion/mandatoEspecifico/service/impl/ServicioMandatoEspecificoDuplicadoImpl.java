...
@Service
public class ServicioMandatoEspecificoDuplicadoImpl implements ServicioMandatoEspecificoDuplicado {

	private static final long serialVersionUID = 5537415850893882049L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioMandatoEspecificoDuplicadoImpl.class);
	...
	public ResultadoImpresionBean imprimirMandatoEspecifico(List<BigDecimal> listaExpediente) {
		...
		try {
			for (BigDecimal numExpediente : listaExpediente) {
				TramiteTrafDuplicadoVO tramite = servicioTramiteTrafico.getTramiteDuplicadoVO(numExpediente, true);
				if (tramite != null) {
					try {
						byte[] bytePdf = imprimir(tramite);
						listaByte.add(bytePdf);
					} catch (Exception e) {
						log.error("Error al imprimir el tramite de duplicado con numExpediente: " + numExpediente, e);
			...
			log.error("Ha sucedido un error a la hora de imprimir los mandatos especificos de Duplicado, error: ", e);
			resultado.setError(Boolean.TRUE);
			resultado.setMensaje("Ha sucedido un error a la hora de imprimir los mandatos especificos de Duplicado.");
		}
		return resultado;
	}

	private byte[] imprimir(TramiteTrafDuplicadoVO tramite) throws ParserConfigurationException, JRException {
	...
	private ArrayList<CampoPdfBean> rellenarPdf(TramiteTrafDuplicadoVO tramite, Set<String> camposPlantilla, ContratoVO contrato) {
	...
	private void rellenarTitular(TramiteTrafDuplicadoVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados) {
	...
	private void rellenarGestor(TramiteTrafDuplicadoVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados, ContratoVO contrato) {
	...
	private void rellenarAsuntos(TramiteTrafDuplicadoVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados) {
		if (camposPlantilla.contains(ConstantesPDF.ID_ASUNTO1_EXPLICACION)) {
			camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_ASUNTO1_EXPLICACION, "DUPLICADO: ", false, false, ConstantesPDF._11));
		}
	...
	private void rellenarDatosFinal(TramiteTrafDuplicadoVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados, ContratoVO contrato) {
