...
@Service
public class ServicioRelacionMatriculasDuplicadoImpl implements ServicioRelacionMatriculasDuplicado {

	private static final long serialVersionUID = -7557628795433365140L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioRelacionMatriculasDuplicadoImpl.class);
	...
	public ResultadoImpresionBean imprimirRelacionMatricula(List<BigDecimal> listaExpediente) {
		...
		try {
			List<TramiteTrafDuplicadoVO> listaTramites = new ArrayList<TramiteTrafDuplicadoVO>();
			for (BigDecimal numExpediente : listaExpediente) {
				listaTramites.add(servicioTramiteTrafico.getTramiteDuplicadoVO(numExpediente, true));
			}
			...
			log.error("Ha sucedido un error a la hora de imprimir las relaciones de matriculas de Duplicado, error: ", e);
			resultado.setError(Boolean.TRUE);
			resultado.setMensaje("Ha sucedido un error a la hora de imprimir las relaciones de matriculas de Duplicado.");
		}
		return resultado;
	}

	private byte[] imprimir(List<TramiteTrafDuplicadoVO> listaTramites) throws ParserConfigurationException, JRException {
	...
	private byte[] generaXML(List<TramiteTrafDuplicadoVO> listaTramites, PdfMaker pdf, boolean mismoTitular, ContratoVO contrato) {
		int numLinea = 0;
		int numeroPagina = 1;
		...
		byte[] bytePdf = null;
		for (TramiteTrafDuplicadoVO tramite : listaTramites) {
	...
	private byte[] procesarMatriculas(TramiteTrafDuplicadoVO tramite, byte[] bytePag, PdfMaker pdf, int numLinea, int numExpedientesEscritos, boolean mismoTitular) {
	...
	private byte[] crearPagina(List<TramiteTrafDuplicadoVO> listaTramites, PdfMaker pdf, boolean mismoTransmitente, ContratoVO contrato, int numeroPagina) {
	...
	private byte[] procesarTitular(List<TramiteTrafDuplicadoVO> listaTramites, byte[] bytePdf, PdfMaker pdf) {
	...
	private byte[] procesarCabecera(List<TramiteTrafDuplicadoVO> listaTramites, byte[] bytePdf, PdfMaker pdf, ContratoVO contrato, int numeroPagina) {
		pdf.establecerFuente(ConstantesPDF.HELVETICA, false, false, ConstantesPDF._10);
	...
	private boolean esMismoTitular(List<TramiteTrafDuplicadoVO> listaTramites) {
	...
	private IntervinienteTraficoVO buscarInterviniente(TramiteTrafDuplicadoVO tramite, String tipoInterviniente) {
	...
	private int getNumExpedientesTotales(List<TramiteTrafDuplicadoVO> listaTramites) {
	...
	private IntervinienteTraficoVO getMismoTitular(List<TramiteTrafDuplicadoVO> listaTramites) {
