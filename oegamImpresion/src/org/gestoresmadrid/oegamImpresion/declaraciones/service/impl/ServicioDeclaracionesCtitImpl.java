...
@Service
public class ServicioDeclaracionesCtitImpl implements ServicioDeclaracionesCtit {

	private static final long serialVersionUID = 7891736302838460783L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioDeclaracionesCtitImpl.class);
	...
	public ResultadoImpresionBean imprimirDeclaracion(List<BigDecimal> listaExpediente, String tipoInterviniente, String tipoImpreso) {
		...
		try {
			for (BigDecimal numExpediente : listaExpediente) {
				...
				if (tramite != null) {
					try {
						byte[] bytePdf = imprimir(tramite, tipoInterviniente, tipoImpreso);
			...
			log.error("Ha sucedido un error a la hora de imprimir la declaración de Ctit, error: ", e);
			resultado.setError(Boolean.TRUE);
			resultado.setMensaje("Ha sucedido un error a la hora de imprimir la declaración de Ctit.");
		}
		return resultado;
	}

	private byte[] imprimir(TramiteTrafTranVO tramite, String tipoInterviniente, String tipoImpreso) {
		PdfMaker pdf = new PdfMaker();
		byte[] bytePdf = pdf.abrirPdf(getRuta(tipoImpreso));
		Set<String> camposPlantilla = pdf.getAllFields(bytePdf);

		ArrayList<CampoPdfBean> camposFormateados = rellenarPdf(tramite, camposPlantilla, pdf, tipoInterviniente, tramite.getContrato());
	...
	private ArrayList<CampoPdfBean> rellenarPdf(TramiteTrafTranVO tramite, Set<String> camposPlantilla, PdfMaker pdf, String tipoInterviniente, ContratoVO contrato) {
		...
		rellenarOtrosDatos(tramite, camposPlantilla, camposFormateados, contrato);

		return camposFormateados;
	}

	private void rellenarOtrosDatos(TramiteTrafTranVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados, ContratoVO contrato) {
		...
		if (camposPlantilla.contains(ConstantesPDF.ID_LUGAR)) {
			...
			camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_LUGAR, nombreMunicipio, false, false, ConstantesPDF._11));
		}
	}
	...
	private void rellenarInterviniente(TramiteTrafTranVO tramite, Set<String> camposPlantilla, ArrayList<CampoPdfBean> camposFormateados, String tipoInterviniente) {
		...
		} else if (PRESENTADOR.equals(tipoInterviniente)) {
			interviniente = buscarInterviniente(tramite, TipoInterviniente.PresentadorTrafico.getValorEnum());
			if (interviniente == null) {
				PersonaVO datosColegiado = servicioPersona.obtenerColegiadoCompletoConDireccion(tramite.getContrato().getColegiado().getNumColegiado(), tramite.getContrato().getColegiado()
						.getUsuario().getNif(), tramite.getContrato().getIdContrato());
				interviniente = new IntervinienteTraficoVO();
				interviniente.setPersona(datosColegiado);
				interviniente.setDireccion(datosColegiado.getDireccion());
			}
		}
		...
		if (interviniente != null) {
			if ("X".equals(interviniente.getPersona().getSexo())) {
				...
				if (camposPlantilla.contains(ConstantesPDF.ID_NIF)) {
					camposFormateados.add(new CampoPdfBean(ConstantesPDF.ID_NIF, interviniente.getPersona().getId().getNif(), false, false, ConstantesPDF._12));
				}
			}
