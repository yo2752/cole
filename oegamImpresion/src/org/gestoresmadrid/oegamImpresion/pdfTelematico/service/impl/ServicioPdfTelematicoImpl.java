...
import org.gestoresmadrid.oegamImpresion.pdfTelematico.service.ServicioPdfTelematico;
...
@Service
public class ServicioPdfTelematicoImpl implements ServicioPdfTelematico {

	private static final long serialVersionUID = 6832531766936363054L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioPdfTelematicoImpl.class);
	...
	@Autowired
	ServicioPdfMatw servicioPdfTelematicoMatw;

	@Autowired
	ServicioPdfTelematicoCtit servicioPdfTelematicoCtit;

	@Autowired
	ServicioPdfTelematicoBaja servicioPdfTelematicoBaja;

	@Override
	public ResultadoImpresionBean imprimirPdfTelematico(Long idImpresion) {
		...
		try {
			...
			if (impresion != null && StringUtils.isNotBlank(impresion.getTipoTramite())) {
				...
				if (numExpedientes != null && !numExpedientes.isEmpty()) {
					if (TipoTramiteTrafico.Matriculacion.getValorEnum().equals(impresion.getTipoTramite())) {
						resultadoImpr = servicioPdfTelematicoMatw.imprimirPdf(numExpedientes, false);
					} else if (TipoTramiteTrafico.TransmisionElectronica.getValorEnum().equals(impresion.getTipoTramite())) {
						resultadoImpr = servicioPdfTelematicoCtit.imprimirPdfTelematico(numExpedientes);
					} else if (TipoTramiteTrafico.Baja.getValorEnum().equals(impresion.getTipoTramite())) {
						resultadoImpr = servicioPdfTelematicoBaja.imprimirPdfTelematico(numExpedientes);
					} else {
					...
					if (resultadoImpr != null && !resultadoImpr.getError()) {
						resultado = servicioImpresion.prepararFinalizarImpresion(numExpedientes, resultadoImpr, getTipoDocumento(), impresion.getContrato(), impresion
								.getTipoTramite(), impresion.getNombreDocumento(), false);
						if (resultadoImpr != null && !resultadoImpr.getError()) {
			...
			log.error("Ha sucedido un error a la hora de imprimir el Pdf Telematico, error: ", e);
			resultadoImpr.setError(Boolean.TRUE);
			resultadoImpr.setMensaje("Ha sucedido un error a la hora de imprimir el Pdf Telematico.");
		}
		return resultadoImpr;
	}

	private String getTipoDocumento() {
		return TipoImpreso.MatriculacionPDFPresentacionTelematica_MATEW.toString();
	}
}
