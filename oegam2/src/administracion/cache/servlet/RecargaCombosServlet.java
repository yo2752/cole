...
public class RecargaCombosServlet extends RecargaCacheServlet{

	private static final long serialVersionUID = 1L;
	private static final ILoggerOegam log = LoggerOegam.getLogger(RecargaCombosServlet.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (validarAcceso(request)) {
			...
			ResultBean resultBean = UtilesVistaTrafico.getInstance().refrescarCombos();
			if (resultBean.getError()) {
				out.println("error");
				log.error("ha ocurrido un error al refrescar los combos en "+InetAddress.getLocalHost().getHostAddress());
			} else {
				out.println("ok");
				log.error("combos refrescados en "+InetAddress.getLocalHost().getHostAddress());
			}
		} else {
