...
<s:iterator value="listaSolicitudes" var="lista" status="statusMatricula">
	...
	<div id="contenedorSolicitudes_<s:property value='#statusMatricula.index' />" class="contenedorSolicitudes">
		...
		<div id="contenedorSolicitud_<s:property value='#statusMatricula.index' />" class="contenedorSolicitud">
			...
			<s:if test="estado < 2">
				<div style="position:absolute; top:.25em; left:95%;">
					<!-- <s:if test="numExpediente != null">
						<a href="javascript:eliminarYrefrescar('<s:property value="numExpediente" />', <s:property value='listaSolicitudes.size()' />);" title="Eliminar esta solicitud">
							<img src="/oegam2/img/matriculas/trash-icon.png" alt="Eliminar" />
						</a>
					</s:if>
					<s:else>
						<a href="javascript:eliminarYrefrescarSinExp(<s:property value='#statusMatricula.index' />, <s:property value='listaSolicitudes.size()' />);" title="Eliminar esta solicitud">
							<img src="/oegam2/img/matriculas/trash-icon.png" alt="Eliminar" />
						</a>
					</s:else> -->
					<a href="javascript:eliminarYrefrescar(<s:property value='#statusMatricula.index' />, <s:property value='listaSolicitudes.size()' />);" title="Eliminar esta solicitud">
						<img src="/oegam2/img/matriculas/trash-icon.png" alt="Eliminar" />
					</a>
				</div>
			</s:if>
			
			<div class="contenedorSolicitudIzquierda">
				<table>
					<tr>
						...
						<td>
							<s:select
								...
								name="listaSolicitudes[%{#statusMatricula.index}].tipoDelantera"
					...
					<tr>
						...
						<td>
							<s:select
								...
								name="listaSolicitudes[%{#statusMatricula.index}].tipoTrasera"
		...
		<div style="clear:both"></div>
			<div class="contenedorSolicitudInferior">
				<div id="preguntaPlacaDuplicada">
					...
					<input type="radio" id="idDuplicadaSi_<s:property value='#statusMatricula.index' />" name="listaSolicitudes[<s:property value='#statusMatricula.index' />].duplicada" value="1"/><label for="listaSolicitudes[<s:property value='#statusMatricula.index' />].duplicada">S&iacute;</label>
					<input type="radio" id="idDuplicadaNo_<s:property value='#statusMatricula.index' />" name="listaSolicitudes[<s:property value='#statusMatricula.index' />].duplicada" value="0"/><label for="listaSolicitudes[<s:property value='#statusMatricula.index' />].duplicada">No;</label>
				</div>	
				...
				<div id="contenedorSolicitudAdicional_<s:property value='#statusMatricula.index' />" class="contenedorSolicitudAdicional">
					<table>
						<tr>
							...
							<td>
								<s:select
									list="@trafico.utiles.UtilesVistaTrafico@getInstance().getTiposPlacasMatriculacion(null)"
									name="listaSolicitudes[%{#statusMatricula.index}].tipoAdicional"
		...
		<div>
			<table>
				<tr>
					<td width="40%" style="text-align:right;">
						<label for="refPropia_<s:property value='#statusMatricula.index' />"><strong>Referencia Propia</strong></label>
					</td>
					<td>
						<s:textfield 
							name="listaSolicitudes[%{#statusMatricula.index}].refPropia"
							id="refPropia_%{#statusMatricula.index}"
							size="50"
							maxlength="500"
							/>
					</td>
	...
	<s:if test="mostrarSeccionPersona == 'true'">
				...
				<tr>
					...
					<td>
						<input type="text" size ="25" maxlength="25" style="position:relative;width:90%;color:#6E6E6E" id="nombre" 
							name="listaSolicitudes[<s:property value='#statusMatricula.index' />].titular.nombre" />
					</td>
					<td style="width:30%" align="right">
						<s:text name="Primer Apellido / Razón Social"></s:text>
					</td>
					<td>
						<input type="text" size ="25" maxlength="25" id="primerApellido" name="listaSolicitudes[<s:property value='#statusMatricula.index' />].titular.apellido1RazonSocial"/>
					</td>
				</tr>
				<tr>	
					<td>
					...
					<td>
						<input type="text" size ="25" maxlength="25" style="position:relative;width:90%;color:#6E6E6E" id="segundoApellido" 
							name="listaSolicitudes[<s:property value='#statusMatricula.index' />].titular.apellido2"/>
					</td>
					<td align="right">
						<s:text name="NIF / CIF"></s:text>
					</td>
					<td>
						<input type="text" disabled="disabled" size ="12" maxlength="12" style="position:relative;width:90%;color:#6E6E6E" id="nif" 
							name="listaSolicitudes[<s:property value='#statusMatricula.index' />].nif" 
							value="<s:property value="listaSolicitudes[#statusMatricula.index].nifTitular"/>"
						/>
					</td>
				</tr>
			</table>	
		...
		<!--<s:hidden id="fechaSolicitud_%{#statusMatricula.index}" name="listaSolicitudes[%{#statusMatricula.index}].fechaSolicitud" value="%{@org.gestoresmadrid.utilidades.components.UtilesFecha@Fecha(fechaSolicitud)}" />-->
		<s:hidden id="estado_%{#statusMatricula.index}" name="listaSolicitudes[%{#statusMatricula.index}].estado" value="%{estado}" />
		<s:hidden id="descEstado_%{#statusMatricula.index}" name="listaSolicitudes[%{#statusMatricula.index}].descEstado" value="%{descEstado}" />
		<s:hidden id="nif_%{#statusMatricula.index}" name="listaSolicitudes[%{#statusMatricula.index}].nifTitular" value="%{nifTitular}" />
	</div>
