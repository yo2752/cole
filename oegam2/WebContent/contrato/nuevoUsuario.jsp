...
<%@ taglib uri="http://displaytag.sf.net" prefix="display"%>
	...
	<table class="subtitulo" cellSpacing="0" cellspacing="0">
		<tr>
			...
			<td>Alta de un nuevo Usuario</td>
		</tr>
	</table>
			...
			<td align="left" nowrap="nowrap">
				<s:if test="%{@org.gestoresmadrid.oegam2.utiles.UtilesVistaAcceso@getInstance().tienePermisoAdmin() || @org.gestoresmadrid.oegam2.utiles.UtilesVistaAcceso@getInstance().tienePermisoColegio()}">
					...
					<s:select list="@trafico.datosSensibles.utiles.UtilesVistaDatosSensibles@getInstance().listarGrupos()"					
						name="usuarioNuevoDto.idGrupo" listKey="idGrupo" listValue="descGrupo" headerKey="-1" headerValue="Ninguno" 						
						id="idGrupoUsuario" />
				</s:if>
				<s:else>
					<s:hidden name="usuarioNuevoDto.idGrupo"/>
				</s:else>
			</td>
		</tr>
	</table>
	<table class="acciones">
		<tr>
			<td>
				<input type="button" class="boton" name="bGuardarUsuario" id="idBotonGuardarUsuario" value="Alta Usuario" onClick="return altaNuevoUsuarioContrato();" onKeyPress="this.onClick"/>
				&nbsp;
			</td>
			<td align="center">
