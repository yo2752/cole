...
function eliminarYrefrescar(numExpediente){
	alert('111numExpediente:'+numExpediente);
...
function eliminarYrefrescar(selSolicitud,idForm){
	
	recargar(selSolicitud,idForm);
}
...
function recargar(){
	...
	var url = "refrescar_AltaSolicitudesPlacas1.action?numsExpedientes=" + numsExpedientes;
	
	var req_monitor = NuevoAjax();
	req_monitor.onreadystatechange = function () {

		if (req_monitor.readyState == 1) {
		...
		if (req_monitor.readyState == 4) {
			if (req_monitor.status == 200) {
...
function recargar(selSolicitud,idForm){

	var formulario = document.getElementById(idForm);
	formulario.action = 'boton_AltaDesolicitudes_AltaSolicitudesPlacas1.action?selSolicitud='+selSolicitud;
	formulario.submit();
