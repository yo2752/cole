...
public class ProcesoLicRegistrarSolicitud extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoLicRegistrarSolicitud.class);
		...
		try {
			...
			if (colaDto == null) {
				...
				if (colaDto.getIdTramite() == null) {
					finalizarTransaccionConErrorNoRecuperableLic(colaDto, "No existen el id del registro de Licencias CAM.");
					colaDto.setRespuesta("No existen el id del registro de Licencias CAM.");
					...
					log.info("Proceso Registro Solicitud Licencia CAM con identificador " + colaDto.getIdTramite());
					ResultBean resultado = servicioLicRegSolicitudRestWS.registrarSolicitudRest(colaDto.getIdTramite(), colaDto.getXmlEnviar(), colaDto.getIdUsuario());
			...
			log.error("Excepcion Proceso Registro de Solicitud de Licencia CAM", e);
	...
	private void finalizarTransaccionConErrorNoRecuperableLic(ColaDto cola, String respuestaError) {
		...
		servicioLicRegSolicitudRestWS.finalizarRegistro(cola.getIdTramite(), respuestaError, new BigDecimal(EstadoLicenciasCam.Finalizado_Con_Error.getValorEnum()), cola.getIdUsuario());
	}

	@Override
	protected String getProceso() {
		return ProcesosEnum.LIC_REGISTRAR_SOLICITUD.getNombreEnum();
	}
}