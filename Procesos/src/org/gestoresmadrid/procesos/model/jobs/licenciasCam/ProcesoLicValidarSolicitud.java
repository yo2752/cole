...
public class ProcesoLicValidarSolicitud extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoLicValidarSolicitud.class);
		...
		try {
			...
			if (colaDto == null) {
				...
				if (colaDto.getIdTramite() == null) {
					...
					log.info("Proceso Validar Solicitud Licencia CAM con identificador " + colaDto.getIdTramite());
					ResultBean resultado = servicioLicEnviarSolicitudRestWS.validarSolicitudRest(colaDto.getIdTramite(), colaDto.getXmlEnviar(), colaDto.getIdUsuario());
			...
			log.error("Excepcion Proceso Validar Solicitud de Licencia CAM", e);
	...
	private void finalizarTransaccionConErrorNoRecuperableLic(ColaDto cola, String respuestaError) {
		...
		servicioLicEnviarSolicitudRestWS.finalizarValidacion(cola.getIdTramite(), respuestaError, new BigDecimal(EstadoLicenciasCam.Finalizado_Con_Error.getValorEnum()), cola.getIdUsuario());
	}

	@Override
	protected String getProceso() {
		return ProcesosEnum.LIC_VALIDAR_SOLICITUD.getNombreEnum();
	}
}