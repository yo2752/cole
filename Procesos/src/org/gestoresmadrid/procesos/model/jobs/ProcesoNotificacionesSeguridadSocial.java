...
public class ProcesoNotificacionesSeguridadSocial extends AbstractProcesoBase {

	private ILoggerOegam log = LoggerOegam.getLogger(ProcesoNotificacionesSeguridadSocial.class);
	...
	protected void doExecute() throws JobExecutionException {
		...
		try {
			...
			log.info(" --INICIO PROCESO DE NOTIFICACIONES_SS--");
			...
			if (solicitud == null) {
				...
				if (log.isInfoEnabled()) {
					log.info("Proceso " + ProcesosEnum.NOTIFICACIONES_SS.getNombreEnum() + " -- Solicitud encontrada [" + solicitud.getIdEnvio() + "]");
				}
				...
				if(solicitud.getXmlEnviar() == null){
					...
					excepcion = "No se ha recuperado el numero de expediente correspondiente a la notificacion de la solicitud con identificador: " + solicitud.getIdEnvio();
					log.error(excepcion);
					try {
					...
					respuesta = servicioConsultaNotificacion.generaRespuesta(solicitud);
			...
			log.error("Ocurrio un error no controlado en el proceso de descarga de notificaciones", e);
			String messageException = e.getMessage() != null ? e.getMessage() : e.toString();
			if (solicitud != null && solicitud.getProceso() != null) {
				try {
					if(messageException.length() > 250){
						messageException = messageException.substring(0, 250);
					}
			...
			log.error("Ocurrio un error no controlado en el proceso de descargar notificaciones de la Seguridad Social al cargar los almacenes de claves", e);
		}
	...
	protected String getProceso() {
		return ProcesosEnum.NOTIFICACIONES_SS.getNombreEnum();
	}
