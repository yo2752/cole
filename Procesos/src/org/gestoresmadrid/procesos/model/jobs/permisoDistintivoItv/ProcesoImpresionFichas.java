...
public class ProcesoImpresionFichas extends AbstractProceso{
	
	private static final ILoggerOegam log = LoggerOegam.getLogger(ProcesoImpresionFichas.class);

	@Autowired
	ServicioImpresionFichasDgt servicioImpresionFichasDgt;
	...
	protected void doExecute() throws JobExecutionException {
		...
		try {
			...
			if(solicitud == null){
				...
				ResultadoPermisoDistintivoItvBean resultado = servicioImpresionFichasDgt.impresionFichasDgt(solicitud);
				if(resultado.getExisteImprPermisosMatw()){
					marcarSolicitudConErrorServicio(solicitud, "No se ha podido ejecutar el proceso de " + ProcesosEnum.IMP_FICHA.getNombreEnum() + " al existir solicitudes encoladas para el proceso de " 
									+ ProcesosEnum.IMP_PRM_MATW.getNombreEnum() + " para el mismo contrato y fecha de presentacion.");
				} else if(resultado != null && resultado.getExcepcion() != null){
			...
			log.error("Ocurrio un error no controlado en el proceso IMPR_FICHAS, error: ", e);
	...
	protected String getProceso() {
		return  ProcesosEnum.IMP_FICHA.getNombreEnum();
	}
	...
	private void finalizarTransaccionCorrectaPermisoDistintivoItv(ColaDto solicitud, String resultadoEjecucion,ResultadoPermisoDistintivoItvBean resultado) {
		...
		servicioImpresionFichasDgt.actualizarEstado(solicitud.getIdTramite(), EstadoPermisoDistintivoItv.GENERADO_JEFATURA, resultadoEjecucion, solicitud.getIdUsuario());
	}
	...
	private void finalizarTransaccionConErrorNoRecuperablePermisoDistintivoItv(ColaDto solicitud, String respuestaError) {
		...
		servicioImpresionFichasDgt.actualizarEstado(solicitud.getIdTramite(), EstadoPermisoDistintivoItv.Finalizado_Error, respuestaError, solicitud.getIdUsuario());
	}
