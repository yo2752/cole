...
public class ProcesoNotificationCTIT extends AbstractProceso {

	private static final ILoggerOegam log = LoggerOegam.getLogger(ProcesoNotificationCTIT.class);

	@Autowired
	ServicioWebServiceNotificacionCtit servicioWebServiceNotificacionCtit;
			...
			try {
				...
				if (solicitud == null) {
					...
					if (solicitud.getXmlEnviar() == null || solicitud.getXmlEnviar().isEmpty()) {
						...
						ResultadoCtitBean resultado = servicioWebServiceNotificacionCtit.tramitarPeticion(solicitud);
				...
				log.error("Excepcion NotificationCTIT", e);
	...
	private void finalizarTransaccionCorrecta(ColaDto cola, String resultadoEjecucion, EstadoTramiteTrafico estadoNuevoProceso) {
		...
		if (EstadoTramiteTrafico.Finalizado_Telematicamente.getValorEnum().equals(estadoNuevoProceso.getValorEnum())) {
			resultActualizacion = servicioWebServiceNotificacionCtit.descontarCreditos(cola.getIdTramite(), cola.getIdUsuario(), cola.getIdContrato());
		}
		if (!resultActualizacion.getError()) {
			resultActualizacion = servicioWebServiceNotificacionCtit.actualizarTramiteProceso(cola.getIdTramite(), estadoNuevoProceso, cola.getIdUsuario(), cola.getRespuesta());
		} else {
	...
	private void finalizarTransaccionConErrorNoRecuperable(ColaDto cola, String mensajeError, EstadoTramiteTrafico estadoNuevoProceso) {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceNotificacionCtit.actualizarTramiteProceso(cola.getIdTramite(), estadoNuevoProceso, cola.getIdUsuario(), cola.getRespuesta());
		if (resultActualizacion.getError()) {
			marcarSolicitudConErrorServicio(cola, resultActualizacion.getMensajeError());
		}
	}
	...
	protected String getProceso() {
		return ProcesosEnum.NOTIFICATIONCTIT.getNombreEnum();
	}
}