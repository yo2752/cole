...
public class ProcesoFullCTIT extends AbstractProceso {

	private static final ILoggerOegam log = LoggerOegam.getLogger(ProcesoFullCTIT.class);

	@Autowired
	ServicioWebServiceFullCtit servicioWebServiceFullCtit;
			...
			try {
				...
				if (solicitud == null) {
					...
					if (solicitud.getXmlEnviar() == null || solicitud.getXmlEnviar().isEmpty()) {
						...
						ResultadoCtitBean resultado = servicioWebServiceFullCtit.tramitarPeticion(solicitud);
				...
				log.error("Excepcion FullCTIT", e);
	...
	private void finalizarTransaccionCorrecta(ColaDto cola, String resultadoEjecucion, EstadoTramiteTrafico estadoNuevoProceso) {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceFullCtit.descontarCreditos(cola.getIdTramite(), cola.getIdUsuario(), cola.getIdContrato());
		if (!resultActualizacion.getError()) {
			resultActualizacion = servicioWebServiceFullCtit.actualizarTramiteProceso(cola.getIdTramite(), estadoNuevoProceso, cola.getIdUsuario(), cola.getRespuesta());
		} else {
	...
	private void finalizarTransaccionConErrorNoRecuperable(ColaDto cola, String mensajeError, EstadoTramiteTrafico estadoNuevoProceso) {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceFullCtit.actualizarTramiteProceso(cola.getIdTramite(), estadoNuevoProceso, cola.getIdUsuario(), cola.getRespuesta());
		if (resultActualizacion.getError()) {
			marcarSolicitudConErrorServicio(cola, resultActualizacion.getMensajeError());
		}
	}
	...
	protected String getProceso() {
		return ProcesosEnum.FULLCTIT.getNombreEnum();
	}
}