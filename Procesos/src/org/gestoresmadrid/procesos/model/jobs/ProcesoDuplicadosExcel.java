...
public class ProcesoDuplicadosExcel extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoDuplicadosExcel.class);

	@Autowired
	ServicioExcelDuplicados servicioExcelDuplicados;
	...
	protected void doExecute() throws JobExecutionException {
		log.info("INICIO PROCESO EXCEL DUPLICADOS");
		...
		try {
			...
			if ("si".equals(gestorPropiedades.valorPropertie("excel.duplicados.lanza.proceso.forzar"))) {
				ejecucion = true;
			} else {
			...
			log.info("No se puede acceder a las propiedades del proceso Excel de Duplicados");
		}
		if (ejecucion) {
			try {
				servicioExcelDuplicados.generarExcelDuplicados();
				actualizarEjecucion(getProceso(), ConstantesProcesos.EJECUCION_CORRECTA, ConstantesProcesos.EJECUCION_CORRECTA, "0");
				peticionCorrecta();
			} catch (Exception e) {
				log.error("Ha sucedido un error a la hora de enviar los duplicados excel, error: ", e);
	...
	protected String getProceso() {
		return ConstantesProcesos.PROCESO_EXCEL_DUPLICADOS;
	}
}