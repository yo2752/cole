...
public class ProcesoVerificacionJustProfSega extends AbstractProcesoBase{

	private static final ILoggerOegam log = LoggerOegam.getLogger(ProcesoVerificacionJustProfSega.class);
	
	private static final String NOTIFICACION = "PROCESO VERIFICACION JUSTIFICANTE PROFESIONAL FINALIZADO";
			...
			try {
				...
				if(solicitud == null){
					...
					if(solicitud.getXmlEnviar() == null || solicitud.getXmlEnviar().isEmpty()){
						...
						ResultadoJustificanteProfesionalBean resultado = servicioWebServiceJustifProfesional.verificacionJustificanteProf(solicitud);
				...
				log.error("Ha sucedido un error en el proceso de verificacion de justificantes profesional, error: ",e);
	...
	protected void finalizarTransaccionConErrorNoRecuperable(ColaBean cola, ResultadoJustificanteProfesionalBean resultado) throws BorrarSolicitudExcepcion {
		...
		ResultadoJustificanteProfesionalBean resultActualizar = servicioWebServiceJustifProfesional.actualizarVerificacionJustProf(cola.getIdTramite(),EstadoJustificante.Finalizado_Con_Error, resultado.getVerificado(),cola.getIdUsuario(),resultado.getListaMensajes());
		if(resultActualizar.isError()){
			marcarSolicitudConErrorServicio(cola, resultActualizar.getMensajeError());
		} else {
	...
	protected void finalizarTransaccionCorrectaJustifProf(ColaBean cola, String resultadoEjecucion, ResultadoJustificanteProfesionalBean resultado) {
		...
		ResultadoJustificanteProfesionalBean resultActualizar = servicioWebServiceJustifProfesional.actualizarVerificacionJustProf(cola.getIdTramite(),EstadoJustificante.Ok, resultado.getVerificado(),cola.getIdUsuario(),resultado.getListaMensajes());
		if(resultActualizar.isError()){
			marcarSolicitudConErrorServicio(cola, resultActualizar.getMensajeError());
		} else {
	...
	protected String getProceso() {
		return ProcesosEnum.VERIFICACIONJPROFSEGA.getNombreEnum();
	}
