...
public class ProcesoFullCtitSega extends AbstractProcesoBase{

	private static final ILoggerOegam log = LoggerOegam.getLogger(ProcesoFullCtitSega.class);
	
	@Autowired
	ServicioWebServiceFullCtitSega servicioWebServiceFullCtitSega;
			...
			try {
				...
				if (colaBean == null) {
					...
					if(colaBean.getXmlEnviar() == null && !colaBean.getXmlEnviar().isEmpty()){
						...
						ResultadoCtitBean resultado = servicioWebServiceFullCtitSega.tramitarFullCtit(colaBean);
				...
				log.error("Ocurrio un error no controlado en el proceso de FullCtitSega, error: ", e);
	...
	private void finalizarTransaccionCorrecta(ColaBean colaBean, String resultadoEjecucion,	EstadoTramiteTrafico estadoNuevoProceso) throws BorrarSolicitudExcepcion {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceFullCtitSega.descontarCreditos(colaBean.getIdTramite(),colaBean.getIdUsuario(), colaBean.getIdContrato());
		if(!resultActualizacion.getError()){
			resultActualizacion = servicioWebServiceFullCtitSega.actualizarTramiteProceso(colaBean.getIdTramite(),estadoNuevoProceso,colaBean.getIdUsuario(),colaBean.getRespuesta());
		} else {
	...
	private void finalizarTransaccionConErrorNoRecuperable(ColaBean colaBean, String mensajeError, EstadoTramiteTrafico estadoNuevoProceso) throws BorrarSolicitudExcepcion {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceFullCtitSega.actualizarTramiteProceso(colaBean.getIdTramite(),estadoNuevoProceso,colaBean.getIdUsuario(),colaBean.getRespuesta());
		if(resultActualizacion.getError()){
			marcarSolicitudConErrorServicio(colaBean, resultActualizacion.getMensajeError());
		}
	}
	...
	protected String getProceso() {
		return ProcesosEnum.FULL_CTIT_SEGA.getNombreEnum();
	}
