...
public class ProcesoTradeCtitSega extends AbstractProcesoBase{

	private static final ILoggerOegam log = LoggerOegam.getLogger(ProcesoTradeCtitSega.class);

	@Autowired
	ServicioWebServiceTradeCtitSega servicioWebServiceTradeCtitSega;
			...
			try {
				...
				if (colaBean == null) {
					...
					if(colaBean.getXmlEnviar() == null && !colaBean.getXmlEnviar().isEmpty()){
						...
						ResultadoCtitBean resultado = servicioWebServiceTradeCtitSega.tramitarTradeCtit(colaBean);
	...
	private void finalizarTransaccionCorrecta(ColaBean colaBean, String resultadoEjecucion,	EstadoTramiteTrafico estadoNuevoProceso) throws BorrarSolicitudExcepcion {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceTradeCtitSega.descontarCreditos(colaBean.getIdTramite(),colaBean.getIdUsuario(), colaBean.getIdContrato());
		if(!resultActualizacion.getError()){
			resultActualizacion = servicioWebServiceTradeCtitSega.actualizarTramiteProceso(colaBean.getIdTramite(),estadoNuevoProceso,colaBean.getIdUsuario(),colaBean.getRespuesta());
		} else {
	...
	private void finalizarTransaccionConErrorNoRecuperable(ColaBean colaBean, String mensajeError, EstadoTramiteTrafico estadoNuevoProceso) throws BorrarSolicitudExcepcion {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceTradeCtitSega.actualizarTramiteProceso(colaBean.getIdTramite(),estadoNuevoProceso,colaBean.getIdUsuario(),colaBean.getRespuesta());
		if(resultActualizacion.getError()){
			marcarSolicitudConErrorServicio(colaBean, resultActualizacion.getMensajeError());
		}
	}
	...
	protected String getProceso() {
		return ProcesosEnum.TRADE_CTIT_SEGA.getNombreEnum();
	}
