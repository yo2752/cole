...
public class ProcesoNotificacionCtitSega  extends AbstractProcesoBase{

	private static final ILoggerOegam log = LoggerOegam.getLogger(ProcesoNotificacionCtitSega.class);
	
	@Autowired
	ServicioWebServiceNotificacionCtitSega servicioWebServiceNotificacionCtitSega;
			...
			try {
				...
				if (colaBean == null) {
					...
					if(colaBean.getXmlEnviar() == null && !colaBean.getXmlEnviar().isEmpty()){
						...
						ResultadoCtitBean resultado = servicioWebServiceNotificacionCtitSega.tramitarNotificacionCtit(colaBean);
				...
				log.error("Ocurrio un error no controlado en el proceso de TradeCtitSega, error: ", e);
	...
	private void finalizarTransaccionCorrecta(ColaBean colaBean, String resultadoEjecucion,	EstadoTramiteTrafico estadoNuevoProceso) throws BorrarSolicitudExcepcion {
		this.finalizarTransaccionCorrecta(colaBean, resultadoEjecucion);
		ResultadoCtitBean resultActualizacion = new ResultadoCtitBean(Boolean.FALSE);
		if(EstadoTramiteTrafico.Finalizado_Telematicamente.getValorEnum().equals(estadoNuevoProceso.getValorEnum())){
			resultActualizacion = servicioWebServiceNotificacionCtitSega.descontarCreditos(colaBean.getIdTramite(),colaBean.getIdUsuario(), colaBean.getIdContrato());
		}
		if(!resultActualizacion.getError()){
			resultActualizacion = servicioWebServiceNotificacionCtitSega.actualizarTramiteProceso(colaBean.getIdTramite(),estadoNuevoProceso,colaBean.getIdUsuario(),colaBean.getRespuesta());
		} else {
			finalizarTransaccionConErrorNoRecuperable(colaBean, resultActualizacion.getMensajeError(), EstadoTramiteTrafico.Finalizado_Con_Error);
		}
		if(resultActualizacion.getError()){
	...
	private void finalizarTransaccionConErrorNoRecuperable(ColaBean colaBean, String mensajeError, EstadoTramiteTrafico estadoNuevoProceso) throws BorrarSolicitudExcepcion {
		...
		ResultadoCtitBean resultActualizacion = servicioWebServiceNotificacionCtitSega.actualizarTramiteProceso(colaBean.getIdTramite(),estadoNuevoProceso,colaBean.getIdUsuario(),colaBean.getRespuesta());
		if(resultActualizacion.getError()){
			marcarSolicitudConErrorServicio(colaBean, resultActualizacion.getMensajeError());
		}
	}
	...
	protected String getProceso() {
		return ProcesosEnum.NOTIFICACION_CTIT_SEGA.getNombreEnum();
	}
