...
public class ProcesoMandatoGenerico extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoMandatoGenerico.class);

	@Autowired
	ServicioMandatoGenerico servicioMandatoGenerico;
		...
		try {
			...
			if (colaDto == null) {
				...
				if (colaDto.getIdTramite() == null) {
					...
					ResultadoImpresionBean resultado = servicioMandatoGenerico.imprimirMandatoGenerico(colaDto.getIdTramite().longValue());
			...
			log.error("Excepcion Proceso Impresion Mandato Generico", e);
	...
	private void enviarCorreo(ResultadoImpresionBean resultado, BigDecimal idImpresion, BigDecimal idContrato, String tipoTramite) {
		servicioImpresion.gestionEnviarCorreo(resultado, idContrato, tipoTramite, TipoImpreso.MatriculacionMandatoGenerico.toString());
	}
