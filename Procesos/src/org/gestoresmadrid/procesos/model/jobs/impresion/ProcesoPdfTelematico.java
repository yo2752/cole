...
public class ProcesoPdfTelematico extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoPdfTelematico.class);

	@Autowired
	ServicioPdfTelematico servicioPdfTelematico;
		...
		try {
			...
			if (colaDto == null) {
				...
				if (colaDto.getIdTramite() == null) {
					...
					ResultadoImpresionBean resultado = servicioPdfTelematico.imprimirPdfTelematico(colaDto.getIdTramite().longValue());
			...
			log.error("Excepcion Proceso Impresion Pdf Telematico", e);
	...
	private void enviarCorreo(ResultadoImpresionBean resultado, BigDecimal idImpresion, BigDecimal idContrato, String tipoTramite) {
		servicioImpresion.gestionEnviarCorreo(resultado, idContrato, tipoTramite, TipoImpreso.MatriculacionPDFPresentacionTelematica_MATEW.toString());
	}
