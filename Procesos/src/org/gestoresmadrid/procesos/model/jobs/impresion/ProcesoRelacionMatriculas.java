...
public class ProcesoRelacionMatriculas extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoRelacionMatriculas.class);

	@Autowired
	ServicioRelacionMatriculas servicioRelacionMatriculas;
		...
		try {
			...
			if (colaDto == null) {
				...
				if (colaDto.getIdTramite() == null) {
					...
					ResultadoImpresionBean resultado = servicioRelacionMatriculas.imprimirRelacionMatrBast(colaDto.getIdTramite().longValue());
			...
			log.error("Excepcion Proceso Impresion Relacion Matricula", e);
	...
	private void enviarCorreo(ResultadoImpresionBean resultado, BigDecimal idImpresion, BigDecimal idContrato, String tipoTramite) {
		servicioImpresion.gestionEnviarCorreo(resultado, idContrato, tipoTramite, TipoImpreso.MatriculacionListadoBastidores.toString());
	}
