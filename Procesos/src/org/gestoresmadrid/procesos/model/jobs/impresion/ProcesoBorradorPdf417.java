...
public class ProcesoBorradorPdf417 extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoBorradorPdf417.class);

	@Autowired
	ServicioBorradorPdf417 servicioBorradorPdf417;
		...
		try {
			...
			if (colaDto == null) {
				...
				if (colaDto.getIdTramite() == null) {
					...
					ResultadoImpresionBean resultado = servicioBorradorPdf417.imprimirBorradorPdf417(colaDto.getIdTramite().longValue());
			...
			log.error("Excepcion Proceso Impresion Borrador Pdf 417", e);
	...
	private void enviarCorreo(ResultadoImpresionBean resultado, BigDecimal idImpresion, BigDecimal idContrato, String tipoTramite) {
		servicioImpresion.gestionEnviarCorreo(resultado, idContrato, tipoTramite, TipoImpreso.MatriculacionBorradorPDF417.toString());
	}
