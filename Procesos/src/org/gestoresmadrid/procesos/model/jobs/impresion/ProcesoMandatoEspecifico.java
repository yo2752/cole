...
public class ProcesoMandatoEspecifico extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoMandatoEspecifico.class);

	@Autowired
	ServicioMandatoEspecifico servicioMandatoEspecifico;
		...
		try {
			...
			if (colaDto == null) {
				...
				if (colaDto.getIdTramite() == null) {
					...
					ResultadoImpresionBean resultado = servicioMandatoEspecifico.imprimirMandatoEspecifico(colaDto.getIdTramite().longValue());
			...
			log.error("Excepcion Proceso Impresion Mandato Especifico", e);
	...
	private void enviarCorreo(ResultadoImpresionBean resultado, BigDecimal idImpresion, BigDecimal idContrato, String tipoTramite) {
		servicioImpresion.gestionEnviarCorreo(resultado, idContrato, tipoTramite, TipoImpreso.MatriculacionMandatoEspecifico.toString());
	}
