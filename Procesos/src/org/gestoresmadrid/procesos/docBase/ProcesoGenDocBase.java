...
public class ProcesoGenDocBase extends AbstractProceso {

	private static ILoggerOegam log = LoggerOegam.getLogger(ProcesoGenDocBase.class);
	...
	protected void doExecute() throws JobExecutionException {
		...
		try {
			...
			if (solicitud == null) {
				...
				ResultadoDocBaseBean resultado = servicioGestionDocBase.procesarPeticionDocBase(solicitud.getIdTramite().longValue(), solicitud.getIdContrato().longValue(),
						solicitud.getXmlEnviar(), solicitud.getIdUsuario().longValue());
	...
	protected String getProceso() {
		return ProcesosEnum.GENERAR_DOC_BASE.getNombreEnum();
	}
	...
	protected void finalizarTransaccionCorrecta(ColaDto cola, String resultado) {
		...
		servicioGestionDocBase.borrarImpresionDocBase(cola.getIdTramite().longValue(), cola.getIdUsuario().longValue());
	}
	...
	protected void finalizarTransaccionErrorRecuperable(ColaDto cola, String respuestaError) {
		...
		if (cola.getnIntento().intValue() < numIntentos.intValue()) {
			...
			servicioGestionDocBase.enviarMailError(cola.getIdTramite().longValue(), cola.getIdContrato().longValue());
