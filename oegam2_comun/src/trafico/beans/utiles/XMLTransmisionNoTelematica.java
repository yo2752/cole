...
import trafico.beans.jaxb.transNoTelematica.DATOS620;
import trafico.beans.jaxb.transNoTelematica.DATOSADQUIRIENTE;
import trafico.beans.jaxb.transNoTelematica.DATOSARRENDADOR;
import trafico.beans.jaxb.transNoTelematica.DATOSCOMPRAVENTA;
import trafico.beans.jaxb.transNoTelematica.DATOSLIMITACION;
import trafico.beans.jaxb.transNoTelematica.DATOSPRESENTACION;
import trafico.beans.jaxb.transNoTelematica.DATOSPRESENTADOR;
import trafico.beans.jaxb.transNoTelematica.DATOSTRANSMITENTE;
import trafico.beans.jaxb.transNoTelematica.DATOSVEHICULO;
import trafico.beans.jaxb.transNoTelematica.DIRECCIONDISTINTAADQUIRIENTE;
import trafico.beans.jaxb.transNoTelematica.FORMATOGA;
import trafico.beans.jaxb.transNoTelematica.TRANSMISION;
import trafico.utiles.UtilesConversiones;
import utilidades.logger.ILoggerOegam;
import utilidades.logger.LoggerOegam;

public class XMLTransmisionNoTelematica {

	private static ILoggerOegam log = LoggerOegam.getLogger(XMLTransmisionNoTelematica.class);
	...
	public XMLTransmisionNoTelematica() {
		SpringBeanAutowiringSupport.processInjectionBasedOnCurrentContext(this);
	}

	/**
	 * Método que obtiene un transmision en formatoGA de un sólo trámite
	 * @param transmision - Trámite de transmision a partir del que generar el documento xml 
	 * */
	public FORMATOGA convertirAFORMATOGA(TramiteTraficoTransmisionBean transmision) {
		...
		ga.getCABECERA().getDATOSGESTORIA().setPROFESIONAL(transmision.getTramiteTraficoBean().getNumColegiado());
		...
		//Añado los trámites
		lista.add(convertirBeanToGa(transmision));
	...
	private FORMATOGA instanciarCompleto() {
		trafico.beans.jaxb.transNoTelematica.ObjectFactory factory = new trafico.beans.jaxb.transNoTelematica.ObjectFactory();
...
//		datos620.setANIOFABRICACION("");
	...
	private TRANSMISION convertirBeanToGa(TramiteTraficoTransmisionBean bean) {
		...
		if(null != tramite.getNumExpediente()) {
			...
			trans.setREFERENCIAPROPIA(null!=tramite.getReferenciaPropia()?tramite.getReferenciaPropia().toString():"");
			...
			trans.setCAMBIOSERVICIO(null!=bean.getCambioServicio()&&
					"true".equals(bean.getCambioServicio())?DecisionTrafico.Si.getValorEnum():"");
			trans.setIMPRESOPERMISO(null!=bean.getImpresionPermiso()&&
					"true".equals(bean.getImpresionPermiso())?DecisionTrafico.Si.getValorEnum():"");
			trans.setRENTING(null!=bean.getTramiteTraficoBean().getRenting()&&
					"true".equals(bean.getTramiteTraficoBean().getRenting())?DecisionTrafico.Si.getValorEnum():"");
			trans.setCONSENTIMIENTO(null!=bean.getConsentimiento()&&
					"true".equals(bean.getConsentimiento())?DecisionTrafico.Si.getValorEnum():"");
			trans.setACTAADJUDICACION(null!=bean.getActaSubasta()&&
					"true".equals(bean.getActaSubasta())?DecisionTrafico.Si.getValorEnum():"");
			trans.setCONTRATOCOMPRAVENTA(null!=bean.getContratoFactura()&&
					"true".equals(bean.getContratoFactura())?DecisionTrafico.Si.getValorEnum():"");
			trans.setSENTENCIAADJUDICACION(null!=bean.getSentenciaJudicial()&&
					"true".equals(bean.getSentenciaJudicial())?DecisionTrafico.Si.getValorEnum():"");
			...
			datosLimitacion.setTIPOLIMITACION(null!=tramite.getIedtm() && "TRUE".equals(tramite.getIedtm().toUpperCase())?"E":"");
		...
		if(null!=adquiriente.getPersona().getNif()) {
			...
			if(null!=persona.getNif()) {
				...
				datosadquiriente.setACTUALIZACIONDOMICILIOADQUIRIENTE(null!=adquiriente.getCambioDomicilio()&&
						"true".equals(adquiriente.getCambioDomicilio())?DecisionTrafico.Si.getValorEnum():"");
				datosadquiriente.setAUTONOMOADQUIRIENTE(null!=adquiriente.getAutonomo()&&
						"true".equals(adquiriente.getAutonomo())?DecisionTrafico.Si.getValorEnum():"");	
				...
				if(null!=direccion.getIdDireccion()) {
					String sigla = null!=direccion.getTipoVia()?
							utilesConversiones.getIdTipoViaDGTFromIdTipoVia(direccion.getTipoVia().getIdTipoVia()):"";
					...
					String letraProvincia = null!=direccion.getMunicipio().getProvincia().getIdProvincia()?
							utilesConversiones.getSiglasFromIdProvincia(direccion.getMunicipio().getProvincia().getIdProvincia()):"";
				...
				if(null!=representanteAdquiriente.getPersona().getNif()) {
					...
					datosadquiriente.setDNIREPRESENTANTEADQUIRIENTE(null!=representanteAdquiriente.getPersona().getNif()?
							representanteAdquiriente.getPersona().getNif():"");
					datosadquiriente.setDOCUMENTOSREPRESENTANTEADQUIRIENTE(null!=representanteAdquiriente.getDatosDocumento()?
							representanteAdquiriente.getDatosDocumento():"");
					...
					datosadquiriente.setAPELLIDO1RAZONSOCIALREPRESENTANTEADQUIRIENTE(null!=representanteAdquiriente.getPersona().getApellido1RazonSocial()?
							representanteAdquiriente.getPersona().getApellido1RazonSocial():"");
					datosadquiriente.setAPELLIDO2REPRESENTANTEADQUIRIENTE(null!=representanteAdquiriente.getPersona().getApellido2()?
							representanteAdquiriente.getPersona().getApellido2():"");
					datosadquiriente.setNOMBREREPRESENTANTEADQUIRIENTE(null!=representanteAdquiriente.getPersona().getNombre()?
							representanteAdquiriente.getPersona().getNombre():"");
					...
					datosadquiriente.setMOTIVOREPRESENTANTEADQUIRIENTE(null!=representanteAdquiriente.getIdMotivoTutela()?representanteAdquiriente.getIdMotivoTutela().getNombreEnum():"");
				...
				if(null!=conductorHabitual.getPersona().getNif()) {
					...
					datosadquiriente.setDNICONDUCTORHABITUAL(null!=conductorHabitual.getPersona().getNif()?
							conductorHabitual.getPersona().getNif():"");
					
					datosadquiriente.setAPELLIDO1RAZONSOCIALCONDUCTORHABITUAL(null!=conductorHabitual.getPersona().getApellido1RazonSocial()?
							conductorHabitual.getPersona().getApellido1RazonSocial():"");
					datosadquiriente.setAPELLIDO2CONDUCTORHABITUAL(null!=conductorHabitual.getPersona().getApellido2()?
							conductorHabitual.getPersona().getApellido2():"");
					datosadquiriente.setNOMBRECONDUCTORHABITUAL(null!=conductorHabitual.getPersona().getNombre()?
							conductorHabitual.getPersona().getNombre():"");
					
					datosadquiriente.setFECHANACIMIENTOCONDUCTORHABITUAL(null!=conductorHabitual.getPersona().getFechaNacimientoBean()?conductorHabitual.getPersona().getFechaNacimientoBean().toString():"");
		...
		if(null!=transmitente.getPersona().getNif()) {
			...
			if(null!=persona.getNif()) {
				...
				datostransmitente.setAUTONOMOTRANSMITENTE(null!=transmitente.getAutonomo()&&
						"true".equals(transmitente.getAutonomo())?DecisionTrafico.Si.getValorEnum():"");
			...
			if(null!=representanteTransmitente.getPersona().getNif()) {
				datostransmitente.setDNIREPRESENTANTETRANSMITENTE(null!=representanteTransmitente.getPersona().getNif()?
						representanteTransmitente.getPersona().getNif():"");
...
//						(null!=representanteTransmitente.getPersona() && null!=representanteTransmitente.getPersona().getApellido2()?representanteTransmitente.getPersona().getApellido2():""));
				...
				datostransmitente.setAPELLIDO1RAZONSOCIALREPRESENTANTETRANSMITENTE(null!=representanteTransmitente.getPersona().getApellido1RazonSocial()?
						representanteTransmitente.getPersona().getApellido1RazonSocial():"");
				datostransmitente.setAPELLIDO2REPRESENTANTETRANSMITENTE(null!=representanteTransmitente.getPersona().getApellido2()?
						representanteTransmitente.getPersona().getApellido2():"");
				datostransmitente.setNOMBREREPRESENTANTETRANSMITENTE(null!=representanteTransmitente.getPersona().getNombre()?
						representanteTransmitente.getPersona().getNombre():"");
				...
				datostransmitente.setMOTIVOREPRESENTANTETRANSMITENTE(null!=representanteTransmitente.getIdMotivoTutela()?representanteTransmitente.getIdMotivoTutela().getNombreEnum():"");
		...
		if(null!=arrendador.getPersona().getNif()) {
			...
			if(null!=persona.getNif()) {
				...
				datosarrendador.setACTUALIZACIONDOMICILIOARRENDADOR(null!=arrendador.getCambioDomicilio()&&
						"true".equals(arrendador.getCambioDomicilio())?DecisionTrafico.Si.getValorEnum():"");
				datosarrendador.setAUTONOMOARRENDADOR(null!=arrendador.getAutonomo()&&
						"true".equals(arrendador.getAutonomo())?DecisionTrafico.Si.getValorEnum():"");	
...
//						(null!=arrendador.getPersona() && null!=arrendador.getPersona().getApellido2()?arrendador.getPersona().getApellido2():""));
			...
			if(null!=representanteArrendador.getPersona().getNif()) {
				datosarrendador.setDNIREPRESENTANTEARRENDADOR(null!=representanteArrendador.getPersona().getNif()?
						representanteArrendador.getPersona().getNif():"");
				...
				datosarrendador.setDOCUMENTOSREPRESENTANTEARRENDADOR(null!=representanteArrendador.getDatosDocumento()?
						representanteArrendador.getDatosDocumento():"");
				datosarrendador.setAPELLIDO1RAZONSOCIALREPRESENTANTEARRENDADOR(null!=representanteArrendador.getPersona().getApellido1RazonSocial()?
						representanteArrendador.getPersona().getApellido1RazonSocial():"");
				datosarrendador.setAPELLIDO2REPRESENTANTEARRENDADOR(null!=representanteArrendador.getPersona().getApellido2()?
						representanteArrendador.getPersona().getApellido2():"");
				datosarrendador.setNOMBREREPRESENTANTEARRENDADOR(null!=representanteArrendador.getPersona().getNombre()?
						representanteArrendador.getPersona().getNombre():"");
				...
				datosarrendador.setMOTIVOREPRESENTANTEARRENDADOR(null!=representanteArrendador.getIdMotivoTutela()?representanteArrendador.getIdMotivoTutela().getNombreEnum():"");
...
//						(null!=representanteArrendador.getPersona() && null!=representanteArrendador.getPersona().getApellido2()?representanteArrendador.getPersona().getApellido2():""));
		...
		if(null!=compraventa.getPersona().getNif()) {
			...
			if(null!=persona.getNif()) {
				...
				datoscompraventa.setACTUALIZACIONDOMICILIOCOMPRAVENTA(null!=compraventa.getCambioDomicilio()&&
						"true".equals(compraventa.getCambioDomicilio())?DecisionTrafico.Si.getValorEnum():"");
				datoscompraventa.setAUTONOMOCOMPRAVENTA(null!=compraventa.getAutonomo()&&
						"true".equals(compraventa.getAutonomo())?DecisionTrafico.Si.getValorEnum():"");	
...
//						(null!=compraventa.getPersona() && null!=compraventa.getPersona().getApellido2()?compraventa.getPersona().getApellido2():""));
			...
			if(null!=representanteCompraVenta.getPersona().getNif()) {
				datoscompraventa.setDNIREPRESENTANTECOMPRAVENTA(null!=representanteCompraVenta.getPersona().getNif()?
						representanteCompraVenta.getPersona().getNif():"");
				...
				datoscompraventa.setDOCUMENTOSREPRESENTANTECOMPRAVENTA(null!=representanteCompraVenta.getDatosDocumento()?
						representanteCompraVenta.getDatosDocumento():"");
				datoscompraventa.setAPELLIDO1RAZONSOCIALREPRESENTANTECOMPRAVENTA(null!=representanteCompraVenta.getPersona().getApellido1RazonSocial()?
						representanteCompraVenta.getPersona().getApellido1RazonSocial():"");
				datoscompraventa.setAPELLIDO2REPRESENTANTECOMPRAVENTA(null!=representanteCompraVenta.getPersona().getApellido2()?
						representanteCompraVenta.getPersona().getApellido2():"");
				datoscompraventa.setNOMBREREPRESENTANTECOMPRAVENTA(null!=representanteCompraVenta.getPersona().getNombre()?
						representanteCompraVenta.getPersona().getNombre():"");
				...
				datoscompraventa.setMOTIVOREPRESENTANTECOMPRAVENTA(null!=representanteCompraVenta.getIdMotivoTutela()?representanteCompraVenta.getIdMotivoTutela().getNombreEnum():"");
...
//			datos620.setANIOFABRICACION(null!=vehiculo.getAnioFabrica()?vehiculo.getAnioFabrica():"");
			...
			datosVehiculo.setDECLARACIONRESPONSABILIDAD(null!=vehiculo.getConsentimiento()?("true".equals(vehiculo.getConsentimiento())?DecisionTrafico.Si.getValorEnum():DecisionTrafico.No.getValorEnum()):"");
			...
			datosVehiculo.setTIPOIDVEHICULO(null!=bean.getTramiteTraficoBean().getVehiculo().getTipoVehiculoBean().getTipoVehiculo()?bean.getTramiteTraficoBean().getVehiculo().getTipoVehiculoBean().getTipoVehiculo().toString():"");

			//DRC@24-09-2012 Incidencia: 2358
			datosVehiculo.setTARA(null!=bean.getTramiteTraficoBean().getVehiculo().getTara()?bean.getTramiteTraficoBean().getVehiculo().getTara().toString():"");			
			datosVehiculo.setPESOMMA(null!=bean.getTramiteTraficoBean().getVehiculo().getPesoMma()?bean.getTramiteTraficoBean().getVehiculo().getPesoMma().toString():"");
			...
			datosVehiculo.setKILOMETRAJE(null!=bean.getTramiteTraficoBean().getVehiculo().getVehiculoTramiteTraficoBean().getKilometros()?bean.getTramiteTraficoBean().getVehiculo().getVehiculoTramiteTraficoBean().getKilometros().toString():"");
			...
			if (null!=direccionVehiculo.getIdDireccion()) {
				...
				String letraProvincia = null!=direccionVehiculo.getMunicipio().getProvincia().getIdProvincia()?
						utilesConversiones.getSiglasFromIdProvincia(direccionVehiculo.getMunicipio().getProvincia().getIdProvincia()):"";
		...
		datos620.setBASEIMPONIBLE(null!=bean.getBaseImponible620()?utiles.bigDecimalToStringDosDecimalesPuntosComas(bean.getBaseImponible620(), 7):"");
		...
		datos620.setEXENTO(null!=bean.getExento620()&&
				"true".equals(bean.getExento620())?DecisionTrafico.Si.getValorEnum():"");
		...
		datos620.setNOSUJETO(null!=bean.getNoSujeto620()&&
				"true".equals(bean.getNoSujeto620())?DecisionTrafico.Si.getValorEnum():"");	
		...
		datos620.setVALORDECLARADO(null!=bean.getValorDeclarado620()?utiles.bigDecimalToStringDosDecimalesPuntosComas(bean.getValorDeclarado620(), 7):"");
		...
		if(direccionDistinta.getIdDireccion()!=null) {
			...
			String letraProvincia = null!=direccionDistinta.getMunicipio().getProvincia().getIdProvincia()?
					utilesConversiones.getSiglasFromIdProvincia(direccionDistinta.getMunicipio().getProvincia().getIdProvincia()):"";
			...
			String sigla = null!=direccionDistinta.getTipoVia()?
					utilesConversiones.getIdTipoViaDGTFromIdTipoVia(direccionDistinta.getTipoVia().getIdTipoVia()):" ";
		...
		datosPresentacion.setJEFATURAPROVINCIAL(null!=bean.getTramiteTraficoBean().getJefaturaTrafico().getJefaturaProvincial()?bean.getTramiteTraficoBean().getJefaturaTrafico().getJefaturaProvincial():"");

//		//Tasas Esperando a ver si se dejan o se quitan.
//		datosPresentacion.setTIPOTASA(null!=bean.getTramiteTraficoBean().getTasa()?bean.getTramiteTraficoBean().getTasa().getTipoTasa():"");
//		datosPresentacion.setTIPOTASACAMBIO(null!=bean.getTipoTasaCambioServicio()?bean.getTipoTasaCambioServicio():"");
//		datosPresentacion.setTIPOTASAINFORME(null!=bean.getTipoTasaInforme()?bean.getTipoTasaInforme():"");
//		datosPresentacion.setCODIGOTASA(null!=bean.getTramiteTraficoBean().getTasa()?bean.getTramiteTraficoBean().getTasa().getCodigoTasa():"");
//		datosPresentacion.setCODIGOTASACAMBIO(null!=bean.getCodigoTasaCambioServicio()?bean.getCodigoTasaCambioServicio():"");
//		datosPresentacion.setCODIGOTASAINFORME(null!=bean.getCodigoTasaInforme()?bean.getCodigoTasaInforme():"");

		//Pagos ITP
		if(null!=bean.getTipoTransferencia())
			if(!bean.getTipoTransferencia().equals("4")) {
				datosPresentacion.setCODIGOELECTRONICOTRANSFERENCIA(null!=bean.getCetItp()?bean.getCetItp():"00000000");
			} else{
				datosPresentacion.setCODIGOELECTRONICOTRANSFERENCIA(null!=bean.getCetItp()?bean.getCetItp():"");
			}
		datosPresentacion.setEXENTOITP(null!=bean.getExentoItp()&&
				"true".equals(bean.getExentoItp())?DecisionTrafico.Si.getValorEnum():"");
		datosPresentacion.setNOSUJETOITP(null!=bean.getNoSujetoItp()&&
				"true".equals(bean.getNoSujetoItp())?DecisionTrafico.Si.getValorEnum():"");
		...
		//Pagos ISD
		datosPresentacion.setEXENTOISD(null!=bean.getExentoIsd()&&
				"true".equals(bean.getExentoIsd())?DecisionTrafico.Si.getValorEnum():"");	
		datosPresentacion.setNOSUJETOISD(null!=bean.getNoSujetoIsd()&&
				"true".equals(bean.getNoSujetoIsd())?DecisionTrafico.Si.getValorEnum():"");	
		...
		datosPresentacion.setEXENTOCEM(null!=bean.getTramiteTraficoBean().getExentoCem()&&
				"true".equals(bean.getTramiteTraficoBean().getExentoCem())?DecisionTrafico.Si.getValorEnum():"");	
		datosPresentacion.setEXENTOIEDMT(null!=bean.getTramiteTraficoBean().getExentoIedtm()&&
				"true".equals(bean.getTramiteTraficoBean().getExentoIedtm())?DecisionTrafico.Si.getValorEnum():"");	
		...
		datosPresentacion.setNOSUJETOIEDMT(null!=bean.getTramiteTraficoBean().getNoSujetoIedtm()&&
				"true".equals(bean.getTramiteTraficoBean().getNoSujetoIedtm())?DecisionTrafico.Si.getValorEnum():"");	

		datosPresentacion.setALTAIVTM(null!=bean.getAltaIvtm()&&"true".equals(bean.getAltaIvtm())?DecisionTrafico.Si.getValorEnum():"");	
		datosPresentacion.setDUA(null!=bean.getDua()&&
				"true".equals(bean.getDua())?DecisionTrafico.Si.getValorEnum():"");	
