...
import org.gestoresmadrid.oegam2comun.sega.model.service.ServicioWebServiceFullCtitSega;
...
import net.gestores.sega.ctit.ws.SoapFullCtitSegaWSHandler;
...
@Service
public class ServicioWebServiceFullCtitSegaImpl implements ServicioWebServiceFullCtitSega{

	private static final long serialVersionUID = -4768148856520922855L;
	
	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioWebServiceFullCtitSegaImpl.class);
	...
	@Override
	public ResultadoCtitBean tramitarFullCtit(ColaBean solicitud) {
			...
			if(xml != null && !xml.isEmpty()){
				...
				if(tramiteTrafTranDto != null){
					...
					if(resultado.getAhiSegundoEnvio()){
						resultado = buildCtitSega.generarXmlSegundoEnvioFullCtit(tramiteTrafTranDto, solicitud.getXmlEnviar());
						if(!resultado.getError()){
							tramiteTrafTranDto.setNpasos(NPasos.Uno.getValorEnum());
							String xmlSegundoEnvio = recogerXml(resultado.getNombreXml(), tramiteTrafTranDto.getNumExpediente());
							if(xmlSegundoEnvio != null && !xmlSegundoEnvio.isEmpty()){
								resultado = llamadaWS(xmlSegundoEnvio, tramiteTrafTranDto, solicitud.getIdUsuario());
							} else {
	...
	private ResultadoCtitBean llamadaWS(String xml, TramiteTrafTranDto tramiteTrafTranDto, BigDecimal idUsuario) {
		...
		try {
			log.info("Proceso " + ProcesosEnum.FULL_CTIT_SEGA.getNombreEnum() + " -- Procesando petición");
			CtitArgument request = completarRequest(xml, tramiteTrafTranDto);
			if(request != null){
				CtitReturn respuesta = getStubFullCtitSega(tramiteTrafTranDto.getNumExpediente()).fullCTIT(request);
				log.info("------" + ProcesosEnum.FULL_CTIT_SEGA.getNombreEnum() + " RECIBIDA RESPUESTA DEL WEBSERVICE-----");
				log.info(ProcesosEnum.FULL_CTIT_SEGA.getNombreEnum() + " customDossierNumber Request:  " + request.getCustomDossierNumber());
				log.info(ProcesosEnum.FULL_CTIT_SEGA.getNombreEnum() + " dossierNumber Response: " + respuesta.getDossierNumber());
				resultadoWS = gestionarRespuesta(respuesta, tramiteTrafTranDto, idUsuario);
			} else {
				resultadoWS.setError(Boolean.TRUE);
				resultadoWS.setMensajeError("Ha sucedido un error a la hora de completar la request de envio al WS de FULL_CTIT_SEGA.");
			}
		} catch (Exception e) {
			log.error("Ha sucedido un error a la hora de realizar la llamada al WS de FULL_CTTIT_SEGA, error: ",e, tramiteTrafTranDto.getNumExpediente().toString());
			resultadoWS.setError(Boolean.TRUE);
	...
	private ResultadoCtitBean gestionarRespuesta(CtitReturn respuesta, TramiteTrafTranDto tramiteTrafTranDto, BigDecimal idUsuario) {
		...
		if(respuesta == null){
			...
			if (CtitStatus._OK.equals(respuesta.getResult().getValue())) {
				if(NPasos.Uno.getValorEnum().equals(tramiteTrafTranDto.getNpasos())){
					resultadoWS.setEstadoNuevoProceso(EstadoTramiteTrafico.Finalizado_Telematicamente);
					resultadoWS.setMensajeError("TRÁMITE OK");
				}else if(NPasos.Dos.getValorEnum().equals(tramiteTrafTranDto.getNpasos())){
					if(TipoConsentimiento.NO.equals(tramiteTrafTranDto.getConsentimiento())){
	...
	private CTITWS getStubFullCtitSega(BigDecimal numExpediente) throws ServiceException, MalformedURLException{
		...
		CTITServiceLocator fullCtitServiceLocator = new CTITServiceLocator();
		javax.xml.namespace.QName portName = new javax.xml.namespace.QName(miURL.toString(),fullCtitServiceLocator.getCTITServiceWSDDServiceName());
		
		@SuppressWarnings("unchecked")
		List<HandlerInfo> list = fullCtitServiceLocator.getHandlerRegistry().getHandlerChain(portName);
		list.add(getFilesHandler(numExpediente));
		stub = (CTITWSBindingStub) fullCtitServiceLocator.getCTITService(miURL);
	...
	private HandlerInfo getFilesHandler(BigDecimal numExpediente) {
		...
		filesConfig.put(SoapFullCtitSegaWSHandler.PROPERTY_KEY_ID, numExpediente);

		// Handler de logs
		HandlerInfo filesHandlerInfo = new HandlerInfo();
		filesHandlerInfo.setHandlerClass(SoapFullCtitSegaWSHandler.class);
	...
	public ResultadoCtitBean descontarCreditos(BigDecimal numExpediente, BigDecimal idUsuario, BigDecimal idContrato) {
		ResultadoCtitBean resultado = new ResultadoCtitBean(Boolean.FALSE);
		try {
			ResultBean resultBean = servicioCredito.descontarCreditos(TipoTramiteTrafico.TransmisionElectronica.getValorEnum(), idContrato, 
					new BigDecimal(1), new BigDecimal(1), ConceptoCreditoFacturado.TTT, numExpediente.toString());
