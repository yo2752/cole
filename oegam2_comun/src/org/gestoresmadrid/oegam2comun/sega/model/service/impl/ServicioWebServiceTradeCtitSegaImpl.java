...
import org.gestoresmadrid.oegam2comun.sega.model.service.ServicioWebServiceTradeCtitSega;
...
import net.gestores.sega.ctit.ws.SoapTradeCtitSegaWSHandler;
...
@Service
public class ServicioWebServiceTradeCtitSegaImpl implements ServicioWebServiceTradeCtitSega{

	private static final long serialVersionUID = 422611455582359064L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(ServicioWebServiceTradeCtitSegaImpl.class);
	...
	@Override
	public ResultadoCtitBean tramitarTradeCtit(ColaBean solicitud) {
	...
	private ResultadoCtitBean llamadaWS(String xml, TramiteTrafTranDto tramiteTrafTranDto, BigDecimal idUsuario) {
		...
		try {
			log.info("Proceso " + ProcesosEnum.TRADE_CTIT_SEGA.getNombreEnum() + " -- Procesando petición");
			CtitArgument request = completarRequest(xml, tramiteTrafTranDto);
			if(request != null){
				CtitReturn respuesta = getStubTradeCtitSega(tramiteTrafTranDto.getNumExpediente()).tradeCTIT(request);
				log.info("------" + ProcesosEnum.TRADE_CTIT_SEGA.getNombreEnum() + " RECIBIDA RESPUESTA DEL WEBSERVICE-----");
				log.info(ProcesosEnum.TRADE_CTIT_SEGA.getNombreEnum() + " customDossierNumber Request:  " + request.getCustomDossierNumber());
				log.info(ProcesosEnum.TRADE_CTIT_SEGA.getNombreEnum() + " dossierNumber Response: " + respuesta.getDossierNumber());
				resultadoWS = gestionarRespuesta(respuesta, tramiteTrafTranDto, idUsuario);
			} else {
				resultadoWS.setError(Boolean.TRUE);
				resultadoWS.setMensajeError("Ha sucedido un error a la hora de completar la request de envio al WS de TRADE_CTTIT_SEGA.");
			}
		} catch (Exception e) {
			log.error("Ha sucedido un error a la hora de realizar la llamada al WS de TRADE_CTTIT_SEGA, error: ",e, tramiteTrafTranDto.getNumExpediente().toString());
			resultadoWS.setError(Boolean.TRUE);
	...
	private CTITWS getStubTradeCtitSega(BigDecimal numExpediente) throws MalformedURLException, ServiceException {
		...
		CTITServiceLocator tradeCtitServiceLocator = new CTITServiceLocator();
		javax.xml.namespace.QName portName = new javax.xml.namespace.QName(miURL.toString(),tradeCtitServiceLocator.getCTITServiceWSDDServiceName());
		
		@SuppressWarnings("unchecked")
		List<HandlerInfo> list = tradeCtitServiceLocator.getHandlerRegistry().getHandlerChain(portName);
		list.add(getFilesHandler(numExpediente));
		stub = (CTITWSBindingStub) tradeCtitServiceLocator.getCTITService(miURL);
	...
	private HandlerInfo getFilesHandler(BigDecimal numExpediente) {
		...
		filesConfig.put(SoapTradeCtitSegaWSHandler.PROPERTY_KEY_ID, numExpediente);

		// Handler de logs
		HandlerInfo filesHandlerInfo = new HandlerInfo();
		filesHandlerInfo.setHandlerClass(SoapTradeCtitSegaWSHandler.class);
