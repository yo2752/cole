...
@Service(value = "modeloInmueblePaginatedImpl")
@Transactional(readOnly = true)
public class ModeloInmueblePaginatedImpl extends AbstractModelPagination {
	...
	public PaginatedList buscarPag(Object beanCriterios, int resPag, int page, String dir, String sort, String[] listInitializedOnePath, String[] listInitializedAnyPath) {
		...
		if (numElementosTotales > 0) {
			...
			if (list != null && !list.isEmpty()) {
				lista = servicioBien.convertirListaEnDto((List<BienVO>) list);
	...
	protected List<AliasQueryBean> crearListaAlias() {
		...
		listaAlias.add(new AliasQueryBean(BienVO.class, "direccion", "direccion", CriteriaSpecification.LEFT_JOIN));
		listaAlias.add(new AliasQueryBean(BienVO.class, "tipoInmueble", "tipoInmueble", CriteriaSpecification.LEFT_JOIN));
		return listaAlias;
	}
	...
	public ServicioBien getServicioBien() {
		return servicioBien;
	}
