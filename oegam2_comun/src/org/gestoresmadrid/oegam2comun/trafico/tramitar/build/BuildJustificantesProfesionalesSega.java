...
@Component
public class BuildJustificantesProfesionalesSega implements Serializable{

	private static final long serialVersionUID = -3100288808664355086L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(BuildJustificantesProfesionalesSega.class);
	...
	public BuildJustificantesProfesionalesSega() {}
	...
	public SolicitudJustificanteProfesional obtenerSolicitudJustificanteSega(JustificanteProfVO justificanteProfVO, ContratoDto contratoDto, IntervinienteTraficoDto titular) {
	...
	private Titular rellenarDatosTitular(IntervinienteTraficoDto titularTramite) {
		...
		if(TipoPersona.Juridica.equals(titularTramite.getPersona().getTipoPersona())){
	...
	private DatosVehiculo rellenarDatosVehiculos(VehiculoVO vehiculoVO) {
		...
		datosVehiculo.setMarca(servicioVehiculo.obtenerNombreMarca(vehiculoVO.getCodigoMarca(), true));
	...
	public ResultBean realizarFirmaSega(SolicitudJustificanteProfesional solicitudJustificanteProfesionalSega, String alias) {
		ResultBean resultFirma = new ResultBean(false);
		try {
			String xmlAfirmar = getXmlAFirmar(solicitudJustificanteProfesionalSega);
			UtilesViafirma utilesViafirma = new UtilesViafirma();
			if(utilesViafirma.firmarPruebaCertificadoCaducidad(xmlAfirmar, alias)==null){
				resultFirma = new ResultBean(true, "Ha sucedido un error a la hora de comprobar la caducidad del certificado para un justificante.");
			}else{
				...
				if (xmlFirmado != null) {
					solicitudJustificanteProfesionalSega.setFirmaGestor(xmlFirmado);
				} else {
	...
	public ResultBean validarYGuardarXml(SolicitudJustificanteProfesional solicitudJustificanteProfesionalSega, BigDecimal numExpediente) {
		ResultBean resultado = new ResultBean(false);
		String nombreXml = ProcesosEnum.JUSTIFICANTES_SEGA.getNombreEnum() + numExpediente;
		File fichero = new File(nombreXml);
		try {
			XMLJustificantesProSegaFactory factory =  new XMLJustificantesProSegaFactory();
			Marshaller marshaller = (Marshaller) factory.getContext().createMarshaller();
			marshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
			marshaller.marshal(solicitudJustificanteProfesionalSega, new FileOutputStream(nombreXml));
			// Lo validamos contra el XSD
			try {
				factory. validarJustificanteProfesional(fichero);
			} catch (OegamExcepcion e) {
				resultado = new ResultBean(true, "Error en la validacion del justificante: " + e.getMensajeError1());
				if(fichero.exists()){
					fichero.delete();
				}
				return resultado;
			...
			gestorDocumentos.guardarFichero(ConstantesGestorFicheros.JUSTIFICANTES, ConstantesGestorFicheros.ENVIO,
					...
					ConstantesGestorFicheros.EXTENSION_XML, new XMLJustificantesProSegaFactory().toXML(solicitudJustificanteProfesionalSega).getBytes("UTF-8"));
