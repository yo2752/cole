...
import org.gestoresmadrid.oegam2comun.sega.matwTipo.view.xml.TipoInspeccion;
...
@Component
public class BuildMatriculacionSega implements Serializable {

	private static final long serialVersionUID = 2902218536085049120L;

	private static final ILoggerOegam log = LoggerOegam.getLogger(BuildMatriculacionSega.class);
	...
	public SolicitudRegistroEntrada obtenerSolicitudRegistroEntrada(TramiteTrafMatrDto tramiteDto, String colegio) {
		...
		if (tramiteDto.getArrendatario() != null)
			if (tramiteDto.getArrendatario().getPersona().getNif() != null && !tramiteDto.getArrendatario().getPersona().getNif().isEmpty()) {
				...
				// TIPO DATO ARRENDATARIO -> Identificación
		...
		if (datosVehiculo.getUsado() != null && datosVehiculo.getUsado().equals(TipoSN.S.value())) {
			...
			// MATRÍCULA ORIGEN
			if (tramiteDto.getVehiculoDto().getMatriculaOrigen() != null && !tramiteDto.getVehiculoDto().getMatriculaOrigen().isEmpty()
					&& TipoTramiteMatriculacion.REMATRICULAR.equals(tramiteDto.getTipoTramiteMatr())) {
		...
		if (tramiteDto.getVehiculoDto().getServicioTrafico() != null && tramiteDto.getVehiculoDto().getServicioTrafico().getIdServicio() != null
				&& !tramiteDto.getVehiculoDto().getServicioTrafico().getIdServicio().isEmpty() && !tramiteDto.getVehiculoDto().getServicioTrafico().getIdServicio().equals("-1")) {
	...
	public SolicitudPruebaCertificado obtenerSolicitudPruebaCertificado(String alias) {
		...
		trafico.beans.jaxb.pruebaCertificado.DatosFirmados datosFirmados = objectFactory.createDatosFirmados();
	...
	private TipoDatoDireccion beanToXmldireccion(DireccionDto direccion) {
		...
		if (direccion != null) {
			...
			if (direccion.getCodPostalCorreos() != null && !direccion.getCodPostalCorreos().isEmpty()) {
				...
				tipoDatoDireccion.setCodigoPostal(new BigInteger("0"));
			}
	...
	private TipoDatoTitular beanToXmlTipoDatoTitular(ObjectFactory objectFactory, IntervinienteTraficoDto interviniente) {
		...
		if (interviniente != null) {
			...
			if (!interviniente.getPersona().getTipoPersona().equals(TipoPersona.Juridica)) {
				if (interviniente.getPersona().getFechaNacimiento() != null && !interviniente.getPersona().getFechaNacimiento().getAnio().isEmpty()
						&& !interviniente.getPersona().getFechaNacimiento().getMes().isEmpty() && !interviniente.getPersona().getFechaNacimiento().getDia().isEmpty())
	...
	private DatosTecnicos completaDatosTecnicos(ObjectFactory objectFactory, TramiteTrafMatrDto tramiteDto) {
		...
		datosTecnicos.setModelo(tramiteDto.getVehiculoDto().getModelo());
		...
		if (tramiteDto.getVehiculoDto().getProcedencia() != null && !tramiteDto.getVehiculoDto().getProcedencia().equals("")
				&& !tramiteDto.getVehiculoDto().getProcedencia().equals("-1") && tramiteDto.getVehiculoDto().getVehiUsado() != null && tramiteDto.getVehiculoDto().getVehiUsado().equals("true")
				&& tramiteDto.getVehiculoDto().getImportado() != null && tramiteDto.getVehiculoDto().getImportado()) {
	...
	private Caracteristicas completaCaracteristicasVehiculo(ObjectFactory objFactory, TramiteTrafMatrDto tramiteDto) {
		...
		if (tramiteDto.getVehiculoDto().getCo2() != null && !tramiteDto.getVehiculoDto().getCo2().isEmpty() && tramiteDto.getVehiculoDto().getCarburante() != null
				&& !"E".equals(tramiteDto.getVehiculoDto().getCarburante())) {
			...
			// No se establece el parámetro ya que no debe de ir en el XML
			valor = tramiteDto.getVehiculoDto().getCo2();
		...
		if (tramiteDto.getVehiculoDto().getMtmaItv() != null && !tramiteDto.getVehiculoDto().getMtmaItv().isEmpty()) {
			BigInteger intMasaTecnicaAdmisible = new BigInteger(tramiteDto.getVehiculoDto().getMtmaItv().toString());
	...
	public ResultBean completarSolicitud(SolicitudRegistroEntrada solicitudRegistroEntrada, String firmado) throws Throwable {
		...
		try {
			...
			//ficheroBean.setSubTipo(ConstantesGestorFicheros.XML_ENVIO);
