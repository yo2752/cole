...
import org.gestoresmadrid.oegam2comun.impresion.excel.cambioServicio.ImpresionExcelCambioServicio;
...
@Service
public class ServicioExcelCambioServicioImpl implements ServicioExcelCambioServicio{

	private static final long serialVersionUID = -380488081569781707L;

	private static ILoggerOegam log = LoggerOegam.getLogger(ServicioExcelCambioServicioImpl.class);
	...
	@Autowired
	ImpresionExcelCambioServicio impresionExcelCambioServicio;
	...
	@Override
	public void generarExcelCambioServicio() {
		ResultadoImpresionExcel resultado = new ResultadoImpresionExcel(Boolean.FALSE);
		if (!servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Madrid.getNombreEnum())) {
			// JEFATURA MADRID
			generarExcel(JefaturasJPTEnum.MADRID,ProcesosEnum.Excel_CambioServicio_Madrid.getNombreEnum(), resultado);
		}
		if (!servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Alcorcon.getNombreEnum())) {
			// JEFATURA ALCORCON
			generarExcel(JefaturasJPTEnum.ALCORCON,ProcesosEnum.Excel_CambioServicio_Alcorcon.getNombreEnum(), resultado);
		}
		if (!servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Alcala.getNombreEnum())) {
			// JEFATURA ALCALA
			generarExcel(JefaturasJPTEnum.ALCALADEHENARES,ProcesosEnum.Excel_CambioServicio_Alcala.getNombreEnum(), resultado);
		}
		if (SI.equalsIgnoreCase(gestorPropiedades.valorPropertie("envio.mail.jefaturaAvila"))
				&& !servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Avila.getNombreEnum())) {
			// JEFATURA AVILA
			generarExcel(JefaturasJPTEnum.AVILA,ProcesosEnum.Excel_CambioServicio_Avila.getNombreEnum(), resultado);
		}

		if (SI.equalsIgnoreCase(gestorPropiedades.valorPropertie("envio.mail.jefaturaCiudadReal"))
				&& !servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Ciudad_Real.getNombreEnum())) {
			// JEFATURA CIUDAD REAL
			generarExcel(JefaturasJPTEnum.CIUDADREAL,ProcesosEnum.Excel_CambioServicio_Ciudad_Real.getNombreEnum(), resultado);
		}

		if (SI.equalsIgnoreCase(gestorPropiedades.valorPropertie("envio.mail.jefaturaCuenca"))
				&& !servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Cuenca.getNombreEnum())) {
			// JEFATURA CUENCA
			generarExcel(JefaturasJPTEnum.CUENCA,ProcesosEnum.Excel_CambioServicio_Cuenca.getNombreEnum(), resultado);
		}

		if (SI.equalsIgnoreCase(gestorPropiedades.valorPropertie("envio.mail.jefaturaSegovia"))
				&& !servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Segovia.getNombreEnum())) {
			// JEFATURA SEGOVIA
			generarExcel(JefaturasJPTEnum.SEGOVIA,ProcesosEnum.Excel_CambioServicio_Segovia.getNombreEnum(), resultado);
		}

		if (SI.equalsIgnoreCase(gestorPropiedades.valorPropertie("envio.mail.jefaturaGuadalajara"))
				&& !servicioProcesos.hayEnvio(ProcesosEnum.Excel_CambioServicio_Guadalajara.getNombreEnum())) {
			// JEFATURA GUADALAJARA
			generarExcel(JefaturasJPTEnum.GUADALAJARA,ProcesosEnum.Excel_CambioServicio_Guadalajara.getNombreEnum(), resultado);
		}
	...
	private StringBuffer getResumenEnvio(ResultadoImpresionExcel resultado) {
		...
		resultadoHtml.append("<br>Resumen de la generación del excel de cambio de servicio con fecha de presentación : ")
	...
	private void generarExcel(JefaturasJPTEnum jefatura, String proceso, ResultadoImpresionExcel resultado) {
		List<TramiteTrafCambioServicioDto> listaCambServicio = servicioTramiteTraficoCambioServicio.cambioServicioExcel(jefatura.getJefatura());
		if (listaCambServicio != null && !listaCambServicio.isEmpty()) {
			...
			impresionExcelCambioServicio.impresionExcelCambioServicio(listaCambServicio, descJefatura, resultado);
		} else {
			log.info("La lista de cambio de servicio para " + jefatura + " esta vacía");
		}
	}
