...
public class ConsultaDevSecurityClientHandler implements Handler {
	...
	private static ILoggerOegam log = LoggerOegam.getLogger(ConsultaDevSecurityClientHandler.class);
	...
	private SOAPEnvelope createFirma(org.apache.axis.MessageContext msgContext) {
		try {
			...
			byte[] firma = utilesViaFirma.firmarConsultasDev(soapEnvelope.getAsString().getBytes(StandardCharsets.UTF_8), alias);
