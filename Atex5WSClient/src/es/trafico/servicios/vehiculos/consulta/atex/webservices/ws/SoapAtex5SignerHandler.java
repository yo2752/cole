 ...
 * 
 */
public class SoapAtex5SignerHandler implements SOAPHandler<SOAPMessageContext> {
	private static ILoggerOegam log = LoggerOegam.getLogger(SoapAtex5SignerHandler.class);
	...
	private SOAPEnvelope createFirma(SOAPMessageContext msgContext) {
		...
		try {
			...
			if (utilesViaFirma.firmarPruebaCertificadoCaducidad(new String(envelopeStringWriter.toString().getBytes(StandardCharsets.UTF_8), StandardCharsets.UTF_8), alias) == null) {
				throw new Exception("No se ha podido comprobar la caducidad de la firma");
			}
			byte[] firma = utilesViaFirma.firmarSoapAtex5(envelopeStringWriter.toString().getBytes(StandardCharsets.UTF_8), alias);
		...
		} catch (IOException | CertificateEncodingException | SOAPException
				| InvalidCanonicalizerException | CanonicalizationException | NoSuchAlgorithmException
				| InvalidAlgorithmParameterException | MarshalException | XMLSignatureException e) {
	...
	public boolean handleMessage(SOAPMessageContext arg0) {
		...
		if (isRequest) {
			if (arg0.containsKey(ALIAS_KEY)) {
				this.alias = (String) arg0.get(ALIAS_KEY);
			}

			SOAPEnvelope signedSoapEnvelope = null;
			signedSoapEnvelope = createFirma(arg0);

			return (signedSoapEnvelope != null);
		}
