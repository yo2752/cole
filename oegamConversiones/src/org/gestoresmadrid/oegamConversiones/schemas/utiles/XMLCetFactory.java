...
@Component
public class XMLCetFactory implements Serializable {

	private static final long serialVersionUID = 1480841365805402757L;

	private static final String NAME_CONTEXT = "org.gestoresmadrid.oegamConversiones.jaxb.cet";
	...
	private static ILoggerOegam log = LoggerOegam.getLogger(XMLCetFactory.class);
	...
	/**
	 * Obtiene el objeto {@link OptiCET620} que contiene los datos del XML que recibe en el String
	 * @param xmlString String con formato XML que contiene la petición realizada
	 * @return Devuelve un objeto {@link OptiCET620} que contiene los datos del XML recibido
	 */
	public OptiCET620 getFormatoOegam2Cet(String xmlString) {
		OptiCET620 formatoDuplicado = null;
		try {
			formatoDuplicado = (OptiCET620) fromXML(xmlString);
		} catch (Exception e) {
			log.error("Error obteniendo getFormatoOegam2Duplicado, error: ", e);
		}
		return formatoDuplicado;
	}
	 ...
	 * @return Devuelve un objeto {@link OptiCET620} que contiene los datos del XML recibido
	 */
	public OptiCET620 getFormatoCet(File fichero) {
		OptiCET620 formatoDuplicado = null;
		try {
			formatoDuplicado = (OptiCET620) fromXML(fichero);
		} catch (Exception e) {
			log.error("Error obteniendo formatoOegam2Duplicado, error: ", e);
		}
		return formatoDuplicado;
	}
	...
	private Object fromXML(String xmlString) throws JAXBException {
		...
		try {
			...
			log.error("Error UNMARSHAL getFormatoOegam2Cet, error: ", e);
		}
		try {
			bais.close();
	...
	private Object fromXML(File fichero) throws JAXBException {
		...
		try {
			...
			log.error("Error UNMARSHAL getFormatoCet, error: ", e);
			log.error(e);
		}
		return obj;
	}
	...
	/**
	 * Método que valida un XML contra el xsd de CET
	 * @param fichero xml a validar
	 * @throws XmlNoValidoExcepcion
	 * @throws IOException
	 * @throws SAXException
	 */
	public void validarXMLFORMATOGACet(File fichero) throws XmlNoValidoExcepcion, IOException {
		URL esquema = this.getClass().getResource("/org/gestoresmadrid/oegamImpex/schemas/cet.xsd");
