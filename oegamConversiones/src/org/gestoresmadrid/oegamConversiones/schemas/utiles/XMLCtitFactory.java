...
@Component
public class XMLCtitFactory implements Serializable {

	private static final long serialVersionUID = -501140903779341289L;

	private static final String NAME_CONTEXT = "org.gestoresmadrid.oegamConversiones.jaxb.ctit";
	...
	private static ILoggerOegam log = LoggerOegam.getLogger(XMLCtitFactory.class);
	...
	/**
	 * Obtiene el objeto {@link FORMATOGA} que contiene los datos del XML que recibe en el String
	 ...
	 */
	public FORMATOGA getFormatoCtit(String xmlString) {
		FORMATOGA formatoCtit = null;
		try {
			formatoCtit = (FORMATOGA) fromXML(xmlString);
		} catch (Exception e) {
			log.error("Error obteniendo FORMATOGACtit, error: ", e);
		}
		return formatoCtit;
	}
	 ...
	 * @return Devuelve un objeto {@link FORMATOGA} que contiene los datos del XML recibido
	 */
	public FORMATOGA getFormatoCtit(File fichero) {
		FORMATOGA formatoCtit = null;
		try {
			formatoCtit = (FORMATOGA) fromXML(fichero);
		} catch (Exception e) {
			log.error("Error obteniendo FORMATOGACtit, error: ", e);
		}
		return formatoCtit;
	...
	/**
	 * Realiza el parseo de los datos XML que se reciben en la cadena de caracteres, almacen�almacenándolos en el objeto que devuelve.
	...
	private Object fromXML(String xmlString) throws JAXBException {
		...
		try {
			...
			log.error("Error UNMARSHAL FORMATOGACtit, error: ", e);
		}
		try {
			bais.close();
	...
	/**
	 * Método que valida un XML contra el xsd de CTIT
	 * @param fichero xml a validar
	 * @throws XmlNoValidoExcepcion
	 * @throws IOException
	 * @throws SAXException
	 */
	public void validarXMLFORMATOGACtit(File fichero, String rutaEsquema) throws XmlNoValidoExcepcion, IOException {
		try {
