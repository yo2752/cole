...
@Component
public class XMLMatwFactory implements Serializable {

	private static final long serialVersionUID = -501140903779341289L;

	private static final String NAME_CONTEXT = "org.gestoresmadrid.oegamConversiones.jaxb.matw";
	...
	private static ILoggerOegam log = LoggerOegam.getLogger(XMLMatwFactory.class);
	 ...
	 */
	public FORMATOGA getFormatoMatw(String xmlString) {
		FORMATOGA formatoMatw = null;
		try {
			formatoMatw = (FORMATOGA) fromXML(xmlString);
		} catch (Exception e) {
			log.error("Error obteniendo FORMATOGAMatw, error: ", e);
		}
		return formatoMatw;
	}
	 ...
	 * @return Devuelve un objeto {@link FORMATOGA} que contiene los datos del XML recibido
	 */
	public FORMATOGA getFormatoMatw(File fichero) {
		FORMATOGA formatoMatw = null;
		try {
			formatoMatw = (FORMATOGA) fromXML(fichero);
		} catch (Exception e) {
			log.error("Error obteniendo FORMATOGAMatw, error: ", e);
		}
		return formatoMatw;
	...
	private Object fromXML(String xmlString) throws JAXBException {
		...
		try {
			...
			log.error("Error UNMARSHAL FORMATOGAMatw, error: ", e);
		}
		try {
			bais.close();
	...
	/**
	 * Método que valida un XML contra el xsd de Matw
	 * @param fichero xml a validar
	 * @throws XmlNoValidoExcepcion
	 * @throws IOException
	 * @throws SAXException
	 */
	public void validarXMLFORMATOGAMatw(File fichero, String rutaEsquema) throws XmlNoValidoExcepcion, IOException {
		try {
